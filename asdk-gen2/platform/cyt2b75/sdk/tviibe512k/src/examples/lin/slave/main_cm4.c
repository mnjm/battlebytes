...
/* LIN Channel in use */
#define CY_LINCH_TYPE           CY_LINCH1_TYPE
#define CY_LINCH_RX_PORT        CY_LINCH1_RX_PORT
#define CY_LINCH_RX_PIN         CY_LINCH1_RX_PIN
#define CY_LINCH_RX_PIN_MUX     CY_LINCH1_RX_PIN_MUX
#define CY_LINCH_TX_PORT        CY_LINCH1_TX_PORT
#define CY_LINCH_TX_PIN         CY_LINCH1_TX_PIN
#define CY_LINCH_TX_PIN_MUX     CY_LINCH1_TX_PIN_MUX
#define CY_LINCH_PCLK           CY_LINCH1_PCLK
#define CY_LINCH_IRQN           CY_LINCH1_IRQN
...
int main(void)
{
     ...
     */
    Cy_SysClk_PeriphAssignDivider(CY_LINCH1_PCLK, CY_SYSCLK_DIV_16_BIT, 0u);
  #if (CY_USE_PSVP == 1)  
...
{
    uint32_t maskStatus;
    Cy_Lin_GetInterruptMaskedStatus(CY_LINCH1_TYPE, &maskStatus);
    Cy_Lin_ClearInterrupt(CY_LINCH1_TYPE, maskStatus);    /* Clear all accepted interrupt. */
    cy_en_lin_status_t apiResponse;

    if ((maskStatus & CY_LIN_INTR_ALL_ERROR_MASK_SLAVE) != 0u)
    {
        /* There are some error */
        /* Handle error if needed. */

        /* Disable the channel to reset LIN status */
        Cy_Lin_Disable(CY_LINCH1_TYPE);
        /* Re-enable the channel */
        Cy_Lin_Enable(CY_LINCH1_TYPE);
        /* Re enable header RX */
        lin_state = LIN_STATE_RX_HEADER;
        Cy_Lin_SetInterruptMask(CY_LINCH1_TYPE, CY_LIN_INTR_RX_HEADER_DONE | CY_LIN_INTR_RX_RESPONSE_DONE | CY_LIN_INTR_ALL_ERROR_MASK_SLAVE);
        Cy_Lin_SetCmd(CY_LINCH1_TYPE, CY_LIN_CMD_RX_HEADER_RX_RESPONSE);
    }
    else
    {
        ...
        case LIN_STATE_RX_HEADER:
            ...
            Cy_Lin_GetHeader(CY_LINCH1_TYPE, &id, &parity);
            /* Analyze ID */
            for (uint8_t i = 0; i < (sizeof(msgContext) / sizeof(msgContext[0])); i++)
            {
            ...
            if (acceptedId)
            {
                ...
                Cy_Lin_SetDataLength(CY_LINCH1_TYPE, msgContext[currentMsgIdx].dataLength);
                Cy_Lin_SetChecksumType(CY_LINCH1_TYPE, msgContext[currentMsgIdx].checksumType);
                if (msgContext[currentMsgIdx].responseDirection == LIN_TX_RESPONSE)
                {
                    Cy_Lin_SetInterruptMask(CY_LINCH1_TYPE, CY_LIN_INTR_TX_RESPONSE_DONE | CY_LIN_INTR_RX_RESPONSE_DONE | CY_LIN_INTR_ALL_ERROR_MASK_SLAVE);
                    Cy_Lin_WriteData(CY_LINCH1_TYPE, msgContext[currentMsgIdx].dataBuffer, msgContext[currentMsgIdx].dataLength);
                    lin_state = LIN_STATE_TX_RESPONSE;
                    Cy_Lin_SetCmd(CY_LINCH1_TYPE, CY_LIN_CMD_TX_RESPONSE);
                }
                else
                {
        ...
        case LIN_STATE_TX_RESPONSE:
            ...
            /* If RX_DONE interrupt occurs, response colision occurs */
        ...
        case LIN_STATE_RX_RESPONSE:
            ...
            while(1)
            {
                apiResponse = Cy_Lin_ReadData(CY_LINCH1_TYPE, msgContext[currentMsgIdx].dataBuffer);
