In order to use this example copy all files from this folder to the /src- folder. Please set the configuration type to be used to Manual Configuration. This can be achieved by setting the macro define in /src/drivers/lpddr4/cy_dramconfsel.h
...
IN contrast to the Solution Designer where all other settings are done in a Solution Designer GUI and expoerted to a header file. With the maual configuration type the user need to set all settings needed to initilaize the controller in the 2 header files.

    cy_dram_config.h
        -PSVP_FREQUENCY
        -PLL Settings --> FSP0/FSP1 Freq.
        -DRAM/ JEDEC Timing Parameters
        -DRAM CHANNEL_DENSITY(some timing parameters depend on this )
        -Burst Length ,WLS,
        -Read/ Write Data Byte Invertion
        -...
        

    -Config_02_RD_LLHH_WR_LLLM_1_AC2.h or 
     Config_02_RD_LLHH_WR_LLLM_1_AC2.h
        -General Controller settings    
        -QoS settings
        -Route Configurations for AXI Ports
        -Address Mapping
        -Timing Registers ( based on what is set in cy_dram_config.h)
        -...
