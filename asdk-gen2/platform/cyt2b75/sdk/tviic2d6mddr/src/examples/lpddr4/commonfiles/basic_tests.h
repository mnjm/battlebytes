/***************************************************************************//**
* \file basic_tests.h
*   $Date: 2022-12-16 20:18:36 +0530 (Fri, 16 Dec 2022) $
*   $Revision: 320217 $
*
...
#define SVN_REV_BASIC_TESTS_H       "$Revision: 320217 $"
#define SVN_DATE_BASIC_TESTS_H      "$Date: 2022-12-16 20:18:36 +0530 (Fri, 16 Dec 2022) $"
        ...
        for(bank_idx=0;bank_idx<2;bank_idx++)
          {
            for(row_idx=0;row_idx<60;row_idx++)
            {
                for(col_idx=0;col_idx<512;col_idx++)
                {
                    ...
                    Cy_Lpddr_GetDramToAxiMapping(LPDDR40, &offset,ch_idx, bank_idx, row_idx, col_idx);
                    ...
                    *(((volatile unsigned int *)CY_LPDDR4_BASE) + offset ) = offset;
                    ...
                    Cy_Lpddr_GetAxiToDramMapping(LPDDR40, rb_offset,&rb_ch, &rb_bank, &rb_row, &rb_col_idx);
...
void InjectECCError()
{
    ...
    volatile uint8_t* ptrECCCode = (uint8_t*)0x80020000;
    volatile uint32_t* ptrECCDatum = (uint32_t*)0x80000000;
    volatile uint32_t datum;
    *ptrECCCode=0x25;
    datum = *ptrECCDatum;

}

uint32_t CheckEntireMemory( )
{
    uint32_t errCount=0;
    for(uint32_t addr= 0x8000000;addr<(0x80000000 + 0x40000000 );addr+=4)
    {
        *(uint32_t*)addr = 0xA5A5A5A5;
    }
...
uint32_t CheckMemoryBlock(uint32_t baseAddress, uint32_t addrOffset )
{
    uint32_t errCount=0;
    for(uint32_t addr= baseAddress;addr<(baseAddress + addrOffset );addr+=4)
    {
        *(uint32_t*)addr = 0xA5A5A5A5;
    }
...
uint32_t CheckMemoryBlockVal(uint32_t baseAddress, uint32_t addrOffset ,uint32_t val)
{
    uint32_t errCount=0;
    for(uint32_t addr= baseAddress;addr<(baseAddress + addrOffset );addr+=4)
    {
...
void WriteAddressToMem(uint32_t addrOffset)
{
    // init error counter
    static uint32_t errCount=0;

    // flag to stop the copy routine
    static bool stopRoutine=false;
    static uint32_t testpattern[5] __attribute__((unused)) = {0xdeadbeaf,0xbadeaffe,0xa5a5a5a5,0x5a5a5a5a,0};
    static uint32_t patternresult[5]__attribute__((unused))= {0,0,0,0,0};
    static uint32_t patternCompleted[5]__attribute__((unused))= {0,0,0,0,0};

    // set the baseaddress where we start the filling memory
    static uint32_t addr=0x80000000;
    // variable for reading back the data 
    volatile uint32_t data=0;
    static uint32_t pattern=0;

    if(stopRoutine==false){
        //as long as we are in a valid range copy data to LPDDR4 RAM
        if(addr>(0x80000000 + addrOffset)){
            addr=0x80000000;
            //save the result of last pattern
            patternresult[pattern]=errCount;
            patternCompleted[pattern]=1;
            errCount=0;
            //next pattern
            pattern++;
            if(pattern>4)
        ...
        //write the actual pattern to memory
        if(pattern<4){
            *(uint32_t*)addr = testpattern[pattern];
        }else{
            *(uint32_t*)addr = addr;
        } 
        // read back the data from memory and compare to currently activate pattern
        data=*(uint32_t*)addr;
        if(pattern<4){
            if(data!=testpattern[pattern])
            {
                errCount++;
            }
        }else
