...
/***************************************************/
/* Short J139_1 with J140_2 on the board */
/***************************************************/
...
#define TEST_READ_LC                    CY_SMIF_SEMP_RD_LATENCY10  // SDR SPI: up to 81MHz, SDR OCTAL: up to 92MHz, DDR OCTAL: up to 85MHz
#define TEST_READ_REG_LC                CY_SMIF_SEMP_REG_LATENCY2 // SDR SPI: up to 133MHz, SDR OCTAL: up to 133MHz, DDR OCTAL: up to 66MHz
...
{
    //{CY_SMIF_CLK_PORT,          CY_SMIF_CLK_PIN,        CY_SMIF_CLK_INV_PIN_MUX,        CY_GPIO_DM_STRONG},
    ...
    {CY_USE_SMIF0_RESET_PORT,   CY_USE_SMIF0_RESET_PIN, CY_USE_SMIF0_RESET_PIN_MUX,     CY_GPIO_DM_PULLUP},    /* Reset based on the GPIO */
};
...
cy_stc_smif_dll_config_t smifDllConfig = 
{
    ...
    .mdlOutDiv  = CY_SMIF_MDL_CLK_OUT_DIV2,
    .mdlTapSel  = CY_SMIF_DDL_1_TAP_DELAY,
...
{
    .xipReadCmdId       = CY_SMIF_SEMP_RDID_1S1S1S_4ADR, // will be updated in the application
    .xipReadMode        = 0x00,                          // Do not care
    .writeEn            = true,
    .xipWriteCmdId      = CY_SMIF_SEMP_WTID_1S1S1S_4ADR, // will be updated in the application
...
/* Register configuration at 100MHz */
uint32_t gPllFrequency = 360000000ul;

/* Please reset the memory device S28Hx before starting using connecting TP85 to GND. */
int main(void)
{
    uint8_t status = 0;
    ...
    /* Modify according to your HW condition. */
    ...
    Cy_SMIF_DeviceSetRxCaptureSdr(SMIF_DEVICE0, CY_SMIF_RX_CAP_STYLE_SDR_POS);
    Cy_SMIF_DeviceSetRxCaptureDdr(SMIF_DEVICE0, CY_SMIF_RX_CAP_STYLE_DDR_OCTAL);
    Cy_SMIF_Set_DelayTapSel(SMIF_DEVICE0, 0);

    Cy_SMIF_DeviceSetDataSelect(SMIF_DEVICE1, CY_SMIF_DATA_SEL0);
    Cy_SMIF_DeviceWriteEnable(SMIF_DEVICE1);
    Cy_SMIF_DeviceSetRxCaptureSdr(SMIF_DEVICE1, CY_SMIF_RX_CAP_STYLE_SDR_POS);
    Cy_SMIF_DeviceSetRxCaptureDdr(SMIF_DEVICE1, CY_SMIF_RX_CAP_STYLE_DDR_OCTAL);
    Cy_SMIF_Set_DelayTapSel(SMIF_DEVICE1, 0);

    /************************************/
    /* 3. Reset the memory connected on SMIF1 */
    /************************************/
    ...
    //writeConfigReg4_0.field.u3IOIMPD = 0x07;    // Impedence -45E
    WriteRegisterSequence(CY_SMIF_SLAVE_SELECT_0,
                          CY_SEMP_REG_ADDR_CFR4_V,
    ...
    writeConfigReg5_0.field.u1SDRDDR = 1; // DDR enabled    // Note!!
    writeConfigReg5_0.field.u1OPI_IT = 1; // Octal Protocol // Note!!
    WriteRegisterSequence(CY_SMIF_SLAVE_SELECT_0,
                          CY_SEMP_REG_ADDR_CFR5_V,
    ...
    cy_en_smif_trx_type_t transMode = CY_SPI_TRANSACTION_8D8D8D; // From here on, this app will do 8D8D8D transaction.

    while(Cy_SMIF_IsBusy(SMIF_USED));
    
    /* Modify according to your HW condition. */
    Cy_SMIF_Disable(SMIF_USED);
    //gPllFrequency = 400000000ul;
    gPllFrequency = 360000000ul;
    ChangePLLFrequency(gPllFrequency);
    smifDllConfig.rxCapMode = CY_SMIF_CAP_MODE_RWDS;
    Cy_SMIF_Init(SMIF_USED, &smifConfig, 1000, &smifContext);
    Cy_SMIF_Enable(SMIF_USED, &smifContext);
    while(Cy_SMIF_IsDllLocked(SMIF_USED) == false);
    ...
    // set up for XIP mode with read/write command
    smifDev0Cfg.addr = CY_SMIF_GetXIP_Address(SMIF_USED);
    smifDev0Cfg.xipReadCmdId  = CY_SMIF_SEMP_RDID_8D8D8D_4ADR;
    smifDev0Cfg.xipWriteCmdId = CY_SMIF_SEMP_WTID_8D8D8D_4ADR;
    CY_ASSERT(Cy_SMIF_InitXIPModeForExMem(SMIF_DEVICE0, CY_SMIF_DEV_OSPI, &smifDev0Cfg, &smifDevice0Context) == CY_SMIF_SUCCESS);

    // write (Please make sure merge mode enabled)
    uint16_t* pbaseAddrXIP_HalfWard = (uint16_t*)smifDev0Cfg.addr;
    uint16_t* pProgramData = (uint16_t*)programData;
    for(uint32_t i_AddrOffset = 0; i_AddrOffset < TEST_PROGRAM_SIZE; i_AddrOffset += DEVICE_PROGRAM_BUFFER_SIZE)
    {
        uint32_t addrOffsetInHalfWard = i_AddrOffset / 2;

        SetModeWithBusyCheck(SMIF_USED, CY_SMIF_NORMAL);
        Cy_SMIF_SEMP_WriteEnable(SMIF_USED, CY_SMIF_SLAVE_SELECT_0, transMode, &smifContext);

        SetModeWithBusyCheck(SMIF_USED, CY_SMIF_MEMORY);
        for(uint32_t i = addrOffsetInHalfWard; i < addrOffsetInHalfWard + (DEVICE_PROGRAM_BUFFER_SIZE / 2); i++)
        {
            pbaseAddrXIP_HalfWard[(TEST_PROGRAM_SECTOR / 2) + i] = pProgramData[i];
        }
    ...
    for(uint32_t i = 0; i < TEST_PROGRAM_SIZE; i++)
    {
        readDataBuffer[i] = pbaseAddrXIP[TEST_PROGRAM_SECTOR+i];
    }
