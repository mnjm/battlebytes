...
#include "lpddr4_memory_init.h"
    ...
    #define VRAM_BASE         (0x24000000U)
    #define VIDEO_RAM_ADDRESS (VRAM_BASE)
#elif defined (tviic2d6mddr)
    #define LPDDR4_BASE       (0x80000000U)
    #define VIDEO_RAM_ADDRESS (LPDDR4_BASE)
#endif
...
uint32_t *refVramAddr = (uint32_t*) VIDEO_RAM_ADDRESS;
...
static void Cy_Mipicsi2_SetInterrupt(void)
{
    ...
    Cy_SysInt_InitIRQ(&irq_cfg); //GFX_2D
...
int main(void)
{
    ...
    /* Set the RAM address for VIDEOSS */
    CY_SET_REG32(0x40A94410UL, VIDEO_RAM_ADDRESS);// CYREG_VIDEOSS0_STORE4_BASEADDRESS
    ...
    for(;;)
    {
        ...
        (void)bitErrStatus;    // For debug
        (void)irqErrStatus;    // For debug
        ...
        if(true == Cy_Lpddr_CheckForRetraining())
        {
            ...
            Cy_SwTmr_Stop(MR4_DQS2DQ_TIMER_ID);
                ...
                (void)cancelCondition;      // For Debug
            #endif
            ...
            (void)mr4ReadInterval;      // For Debug
        }
    }
}
