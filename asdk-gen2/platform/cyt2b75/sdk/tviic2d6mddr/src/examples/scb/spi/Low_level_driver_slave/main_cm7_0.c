...
#define SCB_CH_TO_BE_USED               SCB7
#define SCB_MISO_PORT                   GPIO_PRT4
#define SCB_MISO_PIN                    2
#define SCB_MISO_PIN_MUX                P4_2_SCB7_SPI_MISO

#define SCB_MOSI_PORT                   GPIO_PRT4
#define SCB_MOSI_PIN                    1
#define SCB_MOSI_PIN_MUX                P4_1_SCB7_SPI_MOSI

#define SCB_CLK_PORT                    GPIO_PRT4
#define SCB_CLK_PIN                     0
#define SCB_CLK_PIN_MUX                 P4_0_SCB7_SPI_CLK

#define SCB_SS0_PORT                    GPIO_PRT4
#define SCB_SS0_PIN                     3
#define SCB_SS0_PIN_MUX                 P4_3_SCB7_SPI_SELECT0

#define SCB_CLOCK                       PCLK_SCB7_CLOCK
#define SCB_INTERRUPT_NO                scb_7_interrupt_IRQn
...
uint16_t readData[2];
uint16_t initialWriteData[4] = {0x1122,0x3344, 0x5566, 0x7788};
...
static const cy_stc_scb_spi_config_t SCB_SPI_cfg =
{
    ...
    .txFifoTriggerLevel         = 1,                      /*** When there are fewer entries in the TX FIFO, then at this level the TX trigger output goes high. This output can be connected to a DMA channel through a trigger mux. Also, it controls the \ref CY_SCB_SPI_TX_TRIGGER interrupt source. */
    .txFifoIntEnableMask        = 1,                      /*** Bits set in this mask allow events to cause an interrupt  */
...
void irqSCB(void)
{
    ...
    status = Cy_SCB_SPI_GetTxFifoStatus(SCB_CH_TO_BE_USED);
    if(status & CY_SCB_SPI_TX_TRIGGER)
    {
        /*** Write back the data to TX FIFO ***/
        Cy_SCB_SPI_WriteArray(SCB_CH_TO_BE_USED, (void*)initialWriteData, sizeof(initialWriteData)/sizeof(uint16_t));
        Cy_SCB_SPI_ClearTxFifoStatus(SCB_CH_TO_BE_USED, CY_SCB_SPI_TX_TRIGGER);
    }
}
