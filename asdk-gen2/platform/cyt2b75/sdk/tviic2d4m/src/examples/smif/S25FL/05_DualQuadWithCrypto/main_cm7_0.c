...
int main(void)
{
    ...
    Cy_SMIF_Encrypt(SMIF_USED,(CY_SMIF_GetXIP_Address(SMIF_USED) + TEST_DQUAD_XIP_PROGRAM_ADDR), cryptedProgramData, TEST_PROGRAM_SIZE,&smifContext);
...
static void TestFor_DQUAD_CryptoMMIOWrite_Then_RawAndCryptoXIPRead(uint16_t rd_cmdId)
{
    ...
    for(uint32_t i_WriteOffsetAddrOfQuadDevice = 0; i_WriteOffsetAddrOfQuadDevice < (programSizeInByte / 2); i_WriteOffsetAddrOfQuadDevice+=DEVICE_PROGRAM_BUFFER_SIZE)
    {
        ...
        // Issue program
        Cy_SMIF_MMIO_Program_ExMem(SMIF_USED, D_QUAD_SLAVES, CY_SMIF_S25FL_ID_4QPP, writeStartAddress, (DEVICE_PROGRAM_BUFFER_SIZE*2)/* 512B * 2 */, (uint8_t*)&cryptedProgramData[i_WriteOffsetAddrOfQuadDevice * 2], CY_SMIF_BLOCKING, TEST_SPI_WRITE_LC, 0, &smifDeviceContext, &smifContext);
    ...
    Cy_SMIF_DeviceCryptoDisable(SMIF_DEVICE1);
    ...
    Cy_SMIF_DeviceCryptoEnable(SMIF_DEVICE1);
