...
void BasicWorkFlashNonBlockingTest(uint32_t SectorAddr, uint32_t SectorSizeInByte);
...
int main(void)
{
    ...
    BasicWorkFlashNonBlockingTest(TEST_W_LS_ADDR, CY_WORK_LES_SIZE_IN_BYTE);

    // Test for Small Sector
    BasicWorkFlashNonBlockingTest(TEST_W_SS_ADDR, CY_WORK_SES_SIZE_IN_BYTE);
...
void BasicWorkFlashNonBlockingTest(uint32_t SectorAddr, uint32_t SectorSizeInByte)
{
    ...
    // Verify
    Cy_WorkFlashBlankCheck(SectorAddr, CY_FLASH_DRIVER_NON_BLOCKING);
    ...
    for(uint32_t i_addr = SectorAddr, i_addrOffset = 0; i_addr < SectorAddr + SectorSizeInByte; i_addr+=4, i_addrOffset+=4)
    {
        ...
        // Flash 
        Cy_FlashWriteWork(i_addr, (uint32_t*)&programData[i_dataPos], CY_FLASH_DRIVER_NON_BLOCKING);
