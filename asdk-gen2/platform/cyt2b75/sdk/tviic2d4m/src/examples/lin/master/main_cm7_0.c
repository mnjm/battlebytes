...
int main(void)
{
    ...
    /* LIN baudrate setting */
    {
         ...
         */
        Cy_SysClk_PeriphAssignDivider(LINCH_PCLK, CY_SYSCLK_DIV_16_BIT, 0u);
      #if (CY_USE_PSVP == 1)  
        Cy_SysClk_PeriphSetDivider(Cy_SysClk_GetClockGroup(LINCH_PCLK), CY_SYSCLK_DIV_16_BIT, 0u, 75u);
      #else  
        Cy_SysClk_PeriphSetDivider(Cy_SysClk_GetClockGroup(LINCH_PCLK), CY_SYSCLK_DIV_16_BIT, 0u, 259u); // 80 MHz / 260 / 16 (oversampling) = 19231 Hz
      #endif 
        Cy_SysClk_PeriphEnableDivider(Cy_SysClk_GetClockGroup(LINCH_PCLK), CY_SYSCLK_DIV_16_BIT, 0u);
    }
