...
#include "crypto_hmac_sha3_test_vector.h"

/***********************************************************************************************************************
 * HMAC with SHA3_256
 ***********************************************************************************************************************/
void Run_HMAC_SHA3_256(void)
{
    cy_stc_crypto_context_sha_t cryptoShaContext;

    /* Calculate HASH(s) */
    for (uint32_t i = 0; i < HMAC_SHA3_256_NUMBER_OF_TEST; i++)
    {
        Cy_Crypto_Hmac_Run
        (
            &cryptoCtx,
            &cryptoShaContext,
            &HMAC_SHA3_256_hash[i],
            HMAC_SHA3_256_MsgPtr[i],
            HMAC_SHA3_256_MsgLen[i],
            HMAC_SHA3_256_KeyPtr[i],
            HMAC_SHA3_256_KeyLen[i],
            CY_CRYPTO_MODE_SHA3_256
        );
        /* Wait for completion */
        ...
        CY_ASSERT(memcmp(&HMAC_SHA3_256_hash[i], &HMAC_SHA3_256_hash_ref[i], sizeof(HMAC_SHA3_256_MacLen[i])) == 0);
    }
    /* Then you can get the result from HMAC_SHA3_256_hash */
}
