...
#define RTC_CLK_SRC_LPECO_PRESCALER (2ul)
...
#define RTC_INITIAL_DATE_DOM          (21ul)
#define RTC_INITIAL_DATE_MONTH        (8ul)
#define RTC_INITIAL_DATE_YEAR         (19ul)
...
int main(void)
{
...
#elif (RTC_USES_CLK_SRC == RTC_CLK_SRC_LPECO_PRESCALER)
    #if (CY_SYSTEM_USE_CLOCK != CY_SYSTEM_USE_LPECO)
        BACKUP->unLPECO_CTL.stcField.u1LPECO_EN = 1u;
        while(BACKUP->unLPECO_STATUS.stcField.u1LPECO_READY == 0ul);
    #endif
    /* Set the LPECO Prescaler outpt as the clock source to the RTC block */
    Cy_Rtc_clock_source(CY_RTC_CLK_SRC_LPECO_PRESCALER);
#else
    ...
    for(;;)
    {
        Cy_SysTick_DelayInUs(10000000ul);
        Cy_Rtc_GetDateAndTime(&Read_DateTime);
    }
}
