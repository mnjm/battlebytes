...
#define GFXENV_CONVERT_MEMTYPE_ENUM(memtype)      ( (memtype == CY_GFXENV_EXTMEM_TYPE_S26H)      ? CY_SMIF_HB_MEMTYPE_HYPERFLASH_S26H    : \
                                                    (memtype == CY_GFXENV_EXTMEM_TYPE_S26K)      ? CY_SMIF_HB_MEMTYPE_HYPERFLASH_S26K    : \
                                                    (memtype == CY_GFXENV_EXTMEM_TYPE_S27KXXXX1) ? CY_SMIF_HB_MEMTYPE_HYPERRAM_S27KXXXX1 : \
                                                    (memtype == CY_GFXENV_EXTMEM_TYPE_S27KXXXX2) ? CY_SMIF_HB_MEMTYPE_HYPERRAM_S27KXXXX2 : \
                                                    (cy_smif_hb_memtype_t)0xffff)
...
static cy_gfxenv_en_result_t InitSmifAtSafeFreq(stc_smif_cfg_internal_t * pstcSmifIntCfg, const cy_gfxenv_stc_smif_cfg_t * pstcSmifUsrCfg)
{
    ...
    for(u8Dev = 0; u8Dev < CY_GFXENV_NR_OF_DEVICES_PER_SMIF; u8Dev++)
    {
        ...
        if ((pstcSmifUsrCfg->apstcDevUsrCfg[u8Dev] != NULL) && (pstcSmifUsrCfg->apstcDevUsrCfg[u8Dev]->enExtMemType == CY_GFXENV_EXTMEM_TYPE_S26H))
        {
...
static cy_gfxenv_en_result_t InitSmifDeviceAtSafeFreq(stc_smif_device_cfg_internal_t * pstcDevIntCfg,  const cy_gfxenv_stc_smif_device_cfg_t * pstcDevUsrCfg)
{
    ...
    bool                  bIsHyperFlash = CY_SMIF_HB_IS_HYPERFLASH(GFXENV_CONVERT_MEMTYPE_ENUM(pstcDevUsrCfg->enExtMemType)) ? true : false;
    ...
    if(Cy_SMIF_HB_GetOptimalLatencyCode(u8ExtClkMhz, GFXENV_CONVERT_MEMTYPE_ENUM(pstcDevUsrCfg->enExtMemType), &pstcDevIntCfg->enOptimalLatency) != CY_SMIF_SUCCESS)
    {
        return CY_GFXENV_ERROR_BAD_PARAM;
    }
...
static cy_gfxenv_en_result_t ConfigureHyperFlash(stc_smif_device_cfg_internal_t * pstcDevIntCfg,  const cy_gfxenv_stc_smif_device_cfg_t * pstcDevUsrCfg)
{
    ...
    {
        case CY_GFXENV_EXTMEM_TYPE_S26K:
            ...
            break;
        case CY_GFXENV_EXTMEM_TYPE_S26H:
    ...
    if(pstcDevUsrCfg->enExtMemType == CY_GFXENV_EXTMEM_TYPE_S26H)
    {
...
static cy_gfxenv_en_result_t ConfigureHyperRam(stc_smif_device_cfg_internal_t * pstcDevIntCfg,  const cy_gfxenv_stc_smif_device_cfg_t * pstcDevUsrCfg)
{
    ...
    switch(pstcDevUsrCfg->enExtMemType)
    {
        case CY_GFXENV_EXTMEM_TYPE_S27KXXXX1:
