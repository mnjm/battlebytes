/***************************************************************************//**
* \file system_tviic2d4m_cm0plus.c
* \version 1.0
*
...
#include "cy_project.h"
  ...
  #if (CY_SYS_PLL400M_0_FREQ == CY_SYS_PLL400M_0_240MHz)
    // PLL_OUT = 16,000,000(Feco) / 4 * 120 / 2 = 240,000,000Hz
    // Restriction: 400,000,000 <= Fvco <= 800,000,000
    // This time, Fvco = 16,000,000 * 120 / 4 = 480,000,000.
    #define CY_SYSTEM_PLL0_CONFIG_REFDIV      (4UL)
    #define CY_SYSTEM_PLL0_CONFIG_FEEDBACKDIV (120UL)
    ...
    // PLL_OUT = 16,000,000(Feco) / 4 * 100 / 2 = 200,000,000Hz
    // Restriction: 400,000,000 <= Fvco <= 800,000,000
    // This time, Fvco = 16,000,000 * 100 / 4 = 400,000,000.
    #define CY_SYSTEM_PLL3_CONFIG_REFDIV      (4UL)
    #define CY_SYSTEM_PLL3_CONFIG_FEEDBACKDIV (100UL)
    #define CY_SYSTEM_PLL3_CONFIG_OUTDIV      (2UL)

    // PLL_OUT = 16,000,000(Feco) / 2 * 55 / 2 = 220,000,000Hz
    // Restriction: 400,000,000 <= Fvco <= 800,000,000
  ...
  #if (CY_SYS_PLL400M_0_FREQ == CY_SYS_PLL400M_0_240MHz)
    // PLL_OUT = 8,000,000(Fimo) / 2 * 120 / 2 = 240,000,000Hz
    // Restriction: 400,000,000 < Fvco < 800,000,000
    ...
    // PLL_OUT = 8,000,000(Fimo) / 2 * 133 / 2 = 266,000,000Hz
    // Restriction: 400,000,000 <= Fvco <= 800,000,000
    // This time, Fvco = 8,000,000 * 133 / 2 = 532,000,000.
    #define CY_SYSTEM_PLL2_CONFIG_REFDIV      (2UL)
    #define CY_SYSTEM_PLL2_CONFIG_FEEDBACKDIV (133UL)
    #define CY_SYSTEM_PLL2_CONFIG_OUTDIV      (2UL)

    // PLL_OUT = 8,000,000(Fimo) / 2 * 100 / 2 = 200,000,000Hz
    // Restriction: 400,000,000 <= Fvco <= 800,000,000
    // This time, Fvco = 8,000,000 * 100 / 2 = 400,000,000.
...
void SystemInit (void)
{
    ...
    {
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH6 /* PLL5: PLL200#0 */ }, // setting for clk_hf0
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH1 /* PLL0: PLL400#0 */ }, // setting for clk_hf1
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH7 /* PLL6: PLL200#1 */ }, // setting for clk_hf2
        { .targetDiv = CY_SYSCLK_HFCLK_DIVIDE_BY_2, .source = CY_SYSCLK_HFCLK_IN_CLKPATH6 /* PLL5: PLL200#0 */ }, // setting for clk_hf3
        { .targetDiv = CY_SYSCLK_HFCLK_DIVIDE_BY_4, .source = CY_SYSCLK_HFCLK_IN_CLKPATH6 /* PLL5: PLL200#0 */ }, // setting for clk_hf4
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH2 /* PLL1: PLL400#1 */ }, // setting for clk_hf5
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH2 /* PLL1: PLL400#1 */ }, // setting for clk_hf6
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH2 /* PLL1: PLL400#1 */ }, // setting for clk_hf7
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH3 /* PLL2: PLL400#2 */ }, // setting for clk_hf8
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH3 /* PLL2: PLL400#2 */ }, // setting for clk_hf9
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH4 /* PLL3: PLL400#3 */ }, // setting for clk_hf10
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH5 /* PLL4: PLL400#4 */ }, // setting for clk_hf11
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH7 /* PLL7: PLL200#2 */ }, // setting for clk_hf12
        { .targetDiv = CY_SYSCLK_HFCLK_NO_DIVIDE,   .source = CY_SYSCLK_HFCLK_IN_CLKPATH9 /* IMO */            }, // setting for clk_hf13
    };
    ...
    /* Enable VIDEOSS Power and wait until enabled, then set the global IP Enable bit for VIDEOSS (e.g. to make VRAM accessible) */
    PD->unCTL.stcField.u2PWR_MODE = 3ul;
    while(PD->unSTATUS.stcField.u1PWR_DONE == 0ul);
