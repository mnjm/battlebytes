...
#
# History:
# 2020-09-09 0.01 CHEY  Initial version for silicon
...
# Set the label/address (see GHS manual for supported formats) where the
# debugger will put a breakpoint for the CM7_0 process. Must be a string.
# Note: This breakpoint can typically only be hit after CM7_0 has been enabled by CM0+.
...
#-----------------------------------------------------------------------------------------
# Select whether CM7_0 shall be configured for trace
...
CPU_BOARD_TYPE = 'CPU_BOARD_216_QFP'  # Set 'CPU_BOARD_216_QFP', no other CPU board available at the moment
...
TRACE_PIN_CONFIG = {
                        'CPU_BOARD_216_QFP' : {
                                                'CLK'    : { 'PORT' : 14, 'PIN' : 2, 'HSIOM_SEL' : 26 },
                                                'DATA_0' : { 'PORT' : 14, 'PIN' : 3, 'HSIOM_SEL' : 26 },
                                                'DATA_1' : { 'PORT' : 14, 'PIN' : 4, 'HSIOM_SEL' : 26 },
                                                'DATA_2' : { 'PORT' : 14, 'PIN' : 5, 'HSIOM_SEL' : 26 },
                                                'DATA_3' : { 'PORT' : 14, 'PIN' : 6, 'HSIOM_SEL' : 26 },
                                            }
                    }                        
...
def ConfigureCoreSightTracePaths():
    ...
    if (CM7_0_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED is False):
        return False        
    # Setup all necessary paths as per the user configuration
    if CM7_0_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED:
        ExecDebugCmd("target coresight_path add 2 [CM7_0] -> 4.1 [CSTF: ETM of CM7_0] -> " + ("5 [ETB]" if USE_TPIU_OFFCHIP_TRACE is False else "6 [TPIU]"))
    return True
...
if EXECUTION_MODE == "ERASE":
    if IS_PSVP is True:
        ...
        EraseCodeFlash(0x001f8000, 512*1024)
    else:
        EraseCodeFlash() 
    ResetMcuViaCm0AircrSysResetReqAndHalt()
