...
#define CY_SD_HOST_1_8_REG_STABLE_TIME_MS   (10UL)  /* The 1.8 voltage regulator stable time. */
#define CY_SD_HOST_SUPPLY_RAMP_UP_TIME_MS   (1000UL)  /* The host supply ramp up time. */
...
#if (CY_USE_PSVP == 1u)
    #define CY_SD_HOST_CLK_HF_FREQUENCY           (24000000UL)
#else
    #define CY_SD_HOST_CLK_HF_FREQUENCY           (80000000UL) 
#endif
...
static cy_en_sd_host_status_t Cy_SD_Host_eMMC_InitCard(stc_SDHC_t *base,
                                             cy_stc_sd_host_sd_card_config_t const *config,
                                             cy_stc_sd_host_context_t *context);
//static cy_en_sd_host_status_t Cy_SD_Host_CmdRxData(stc_SDHC_t *base, cy_stc_sd_host_data_config_t *pcmd);
// __STATIC_INLINE cy_en_sd_host_status_t Cy_SD_Host_CmdTxData(stc_SDHC_t *base, cy_stc_sd_host_data_config_t *pcmd);
static cy_en_sd_host_status_t Cy_SD_Host_OpsGoIdle(stc_SDHC_t *base);
static cy_en_sd_host_status_t Cy_SD_Host_OpsVoltageSwitch(stc_SDHC_t *base,
                                                          cy_stc_sd_host_context_t const *context);
static cy_en_sd_host_status_t Cy_SD_Host_OpsSwitchFunc(stc_SDHC_t *base, 
                                                       uint32_t cmdArgument);

__STATIC_INLINE cy_en_sd_host_status_t Cy_SD_Host_OpsProgramCsd(stc_SDHC_t *base, 
                                                                uint32_t csd,
                                                                cy_stc_sd_host_context_t *context);
static cy_en_sd_host_status_t Cy_SD_Host_OpsSelectCard(stc_SDHC_t *base, 
                                                      cy_stc_sd_host_context_t const *context);
static cy_en_sd_host_status_t Cy_SD_Host_OpsSendIoRwDirectCmd(stc_SDHC_t *base, 
                                                            uint32_t rwFlag,
                                                            uint32_t functionNumber,
                                                            uint32_t rawFlag,
                                                            uint32_t registerAaddress,
                                                            uint32_t data);
static cy_en_sd_host_status_t Cy_SD_Host_OpsSendAppCmd(stc_SDHC_t *base, 
                                                       cy_stc_sd_host_context_t const *context);
__STATIC_INLINE cy_en_sd_host_status_t Cy_SD_Host_OpsSendIfCond(stc_SDHC_t *base, 
                                                       uint32_t cmdArgument,
                                                       bool noResponse);
__STATIC_INLINE cy_en_sd_host_status_t Cy_SD_Host_OpsSetSdBusWidth(stc_SDHC_t *base, 
                                                        uint32_t cmdArgument,
                                                        cy_stc_sd_host_context_t const *context);
static cy_en_sd_host_status_t Cy_SD_Host_OpsSdioSendOpCond(stc_SDHC_t *base, 
                                                           uint32_t *ocrReg,
                                                           uint32_t cmdArgument);
static cy_en_sd_host_status_t Cy_SD_Host_OpsSdSendOpCond(stc_SDHC_t *base, 
                                                        uint32_t *ocrReg,
                                                        uint32_t cmdArgument,
                                                        cy_stc_sd_host_context_t const *context);
__STATIC_INLINE cy_en_sd_host_status_t Cy_SD_Host_MmcOpsSendOpCond(stc_SDHC_t *base,
                                                        uint32_t *ocrReg,
                                                        uint32_t cmdArgument,
                                                        cy_stc_sd_host_context_t *context);
...
static void Cy_SD_Host_ErrorReset(stc_SDHC_t *base);
static void Cy_SD_Host_NormalReset(stc_SDHC_t *base);
__STATIC_INLINE bool Cy_SD_Host_VoltageCheck(stc_SDHC_t *base);
__STATIC_INLINE cy_en_sd_host_status_t Cy_SD_Host_IoOcr(stc_SDHC_t *base,
                                      bool lowVoltageSignaling,
                                      uint32_t *s18aFlag,
                                      uint32_t *sdioFlag,
                                      bool *mpFlag,                                   
                                      uint32_t *ocrReg);
__STATIC_INLINE cy_en_sd_host_status_t Cy_SD_Host_SdOcr(stc_SDHC_t *base,
                                      bool lowVoltageSignaling,
                                      uint32_t *s18aFlag,
                                      bool *mpFlag,
                                      bool f8Flag,
                                                       ...
                                                       cy_stc_sd_host_context_t *context)
{
    ...
    //cy_en_sd_host_bus_speed_mode_t speedMode = CY_SD_HOST_BUS_SPEED_EMMC_LEGACY;                                      // Old legacy speed mode
    cy_en_sd_host_bus_speed_mode_t speedMode = CY_SD_HOST_BUS_SPEED_EMMC_HIGHSPEED_SDR;                                 // High Speed SDR upto 50M
    uint32_t retry;
    ...
    //if (CY_SD_HOST_SUCCESS == ret) /* The ready state. */
    {
        ...
        {
            /* Set RCA (CMD3) */
            ret = Cy_SD_Host_SetRca(base, config->rca);
            context->RCA = *(config->rca);
                ...
                if (CY_SD_HOST_SUCCESS == ret) /* The transfer state. */
                {
                    ...
                    */
                    ...
                    { 
                        ...
                        if (CY_SD_HOST_SUCCESS == ret)
                        {
                            //(void)Cy_SD_Host_SdCardChangeClock(base, CY_SD_HOST_CLK_20M);                             // Old Low speed setting
                            (void)Cy_SD_Host_SdCardChangeClock(base, CY_SD_HOST_CLK_40M);
                        }
...
* to the VDDIO of the SD block in order to operate at 1.8V.
* The Cy_SD_Host_InitCard() function is not applicable when the user needs 
* to configure the custom IO pin to control (using the GPIO driver)
* the regulator supplying voltage. Before using the signal voltage switch with
* the custom IO pin, the card must be set to Ready state using the CMD0, CMD8 and ACMD41
* commands sequence. Refer to the 3.6.1 section of "Part A2 SD Host Controller Specification" 
* about the signal voltage switch procedure.
*
*******************************************************************************/
...
cy_en_sd_host_status_t Cy_SD_Host_PollCmdLineFree(stc_SDHC_t const *base)
{
    ...
    while (retry > 0UL)
    {
        commandInhibit = _FLD2BOOL(SDHC_CORE_PSTATE_REG_CMD_INHIBIT,
                               base->CORE.unPSTATE_REG.u32Register);
...
cy_en_sd_host_status_t Cy_SD_Host_PollDataLineFree(stc_SDHC_t const *base)
{
    ...
    while ((true == _FLD2BOOL(SDHC_CORE_PSTATE_REG_DAT_LINE_ACTIVE, base->CORE.unPSTATE_REG.u32Register)) && 
           (retry > 0UL))
    {
    ...
    if (true == _FLD2BOOL(SDHC_CORE_PSTATE_REG_DAT_LINE_ACTIVE, base->CORE.unPSTATE_REG.u32Register))
    {
...
cy_en_sd_host_status_t Cy_SD_Host_PollBufferReadReady(stc_SDHC_t *base)
{
    ...
    while (retry > 0UL)
    {
                            ...
                            base->CORE.unNORMAL_INT_STAT_R.u16Register)) 
        {
            ...
            base->CORE.unNORMAL_INT_STAT_R.u16Register = CY_SD_HOST_BUF_RD_READY;
...
// *
// *******************************************************************************/
// __STATIC_INLINE cy_en_sd_host_status_t Cy_SD_Host_CmdTxData(stc_SDHC_t *base,
                                                   // cy_stc_sd_host_data_config_t *pcmd)
// {
    // cy_en_sd_host_status_t ret;
    // uint32_t               blkSize;
    ...
    // {
        ...
        // while ((CY_SD_HOST_SUCCESS != ret) && (0UL < retry))
        // {
            // ret = Cy_SD_Host_PollTransferComplete(base);
            // retry--;
        // }
    ...
    if ((NULL != base) && (NULL != config))
    {
        ...
        if (CY_SD_HOST_SUCCESS == ret)
        {
            ...
            if (CY_SD_HOST_SUCCESS == ret)
            {
                ...
                base->CORE.unARGUMENT_R.u32Register = config->commandArgument;

                /* Update the command hardware register (the command is sent)
                * according to the spec, this register should be written only once.
                */
                    base->CORE.unCMD_R.u16Register = (uint16_t)(_VAL2FLD(SDHC_CORE_CMD_R_CMD_TYPE, 0U) |
    ...
    if ((CY_SD_HOST_SUCCESS == ret) && (CY_SD_HOST_SDIO != context->cardType))
    {
        ...
        if (0UL == pState)
        {
            ...
            if (true == _FLD2BOOL(SDHC_CORE_HOST_CTRL2_R_SIGNALING_EN,
                                  base->CORE.unHOST_CTRL2_R.u16Register))
            {
...
*
*******************************************************************************/
static cy_en_sd_host_status_t Cy_SD_Host_OpsSwitchFunc(stc_SDHC_t *base, uint32_t cmdArgument)
{
    cy_stc_sd_host_cmd_config_t cmd;
    cy_en_sd_host_status_t      ret;
                                                                ...
                                                                cy_stc_sd_host_context_t *context)
{
    ...
    while ((true == _FLD2BOOL(SDHC_CORE_PSTATE_REG_DAT_LINE_ACTIVE, base->CORE.unPSTATE_REG.u32Register)) && (retry > 0UL) )
    {
        Cy_SysLib_DelayUs(CY_SD_HOST_WRITE_TIMEOUT_MS);
        retry--;
    }
...
cy_en_sd_host_status_t Cy_SD_Host_SdCardChangeClock(stc_SDHC_t *base, uint32_t frequency)
{
    cy_en_sd_host_status_t ret = CY_SD_HOST_ERROR_INVALID_PARAMETER;
    uint32_t               clkDiv;
    uint32_t               clockInput = CY_SD_HOST_CLK_HF_FREQUENCY;
                      ...
                      cy_stc_sd_host_context_t *context)
{
    ...
    if ((NULL != base) && (NULL != config) && (NULL != context))
    {
        ...
        base->CORE.unGP_OUT_R.u32Register = _VAL2FLD(SDHC_CORE_GP_OUT_R_IO_VOLT_SEL_OE, 1u) | /* The IO voltage selection signal. */
        ...
        if (config->emmc)
        {
            ...
            base->CORE.unEMMC_CTRL_R.u16Register = (uint16_t)_CLR_SET_FLD16U(base->CORE.unEMMC_CTRL_R.u16Register,
        ...
        {                          
            /* LED Control. */
            base->CORE.unHOST_CTRL1_R.u8Register = (uint8_t)_CLR_SET_FLD16U(base->CORE.unHOST_CTRL1_R.u8Register,
                                          SDHC_CORE_HOST_CTRL1_R_LED_CTRL,
                                          1u);
        }

        /* Select ADMA or not */
        base->CORE.unHOST_CTRL1_R.u8Register = (uint8_t)_CLR_SET_FLD8U(base->CORE.unHOST_CTRL1_R.u8Register, 
                                      SDHC_CORE_HOST_CTRL1_R_DMA_SEL, 
                                      config->dmaType);
                                      
            /* Set data timeout time to max */
        base->CORE.unTOUT_CTRL_R.u8Register = _CLR_SET_FLD8U(base->CORE.unTOUT_CTRL_R.u8Register,
        ...
        base->CORE.unHOST_CTRL2_R.u16Register = (uint16_t)_CLR_SET_FLD16U(base->CORE.unHOST_CTRL2_R.u16Register, 
    ...
    if (NULL != base)
    {
        if (CY_SD_HOST_SUCCESS ==  Cy_SD_Host_PollCmdLineFree(base))
        {
            if (CY_SD_HOST_SUCCESS ==  Cy_SD_Host_PollDataLineFree(base))
            {
                ...
                base->WRAP.unCTL.u32Register = _CLR_SET_FLD32U(base->WRAP.unCTL.u32Register,
...
*
*******************************************************************************/
uint32_t Cy_SD_Host_GetCardStatus(stc_SDHC_t *base,
                                  cy_stc_sd_host_context_t const *context)
{
    cy_stc_sd_host_cmd_config_t cmd;
    cy_en_sd_host_status_t      ret = CY_SD_HOST_ERROR_INVALID_PARAMETER;
...
*
*******************************************************************************/
uint32_t Cy_SD_Host_GetOcr(stc_SDHC_t *base, cy_stc_sd_host_context_t *context)
{
    uint32_t ocrReg = 0UL;
...
*
*******************************************************************************/
cy_en_sd_host_status_t Cy_SD_Host_GetCid(stc_SDHC_t *base, 
                                         uint32_t *cid)
{
    cy_stc_sd_host_cmd_config_t cmd;
    cy_en_sd_host_status_t      ret = CY_SD_HOST_ERROR_INVALID_PARAMETER;
                                         ...
                                         cy_stc_sd_host_context_t *context)
{
    ...
    if ((NULL != base) && (NULL != csd) && (NULL != context))
    {
        ...
        if (ret == CY_SD_HOST_SUCCESS)
        {
            ...
            if (CY_SD_HOST_SDHC == context->cardCapacity) /* High/Ext. Capacity (CSD version 2.0) */
            {
...
*
*******************************************************************************/
cy_en_sd_host_status_t Cy_SD_Host_GetExtCsd(stc_SDHC_t *base, uint32_t *extCsd,
                                                   cy_stc_sd_host_context_t *context)
{
    cy_stc_sd_host_cmd_config_t  cmd;
    cy_en_sd_host_status_t       ret = CY_SD_HOST_ERROR_INVALID_PARAMETER;
    ...
    if ((NULL != base) && (NULL != extCsd) && (NULL != context))
    {
        ...
        cmd.commandArgument = 0;					//->RCA << CY_SD_HOST_RCA_SHIFT;                            
        cmd.dataPresent     = true;
        cmd.enableAutoResponseErrorCheck = false;
        cmd.respType        = CY_SD_HOST_RESPONSE_LEN_48;
        cmd.enableCrcCheck  = true;
...
*
* \note This function can be used only if the card is in the Indentification or 
* Stand-by state.
*
* \param *base
*     The SD host registers structure pointer.
...
/*******************************************************************************
* Function Name: Cy_SD_Host_SetRca
****************************************************************************//**
*
*  Sets the Relative Card Address (RCA) to the Card/eMMC register
*
* \note This function can be used only if the card is in the Indentification or 
* Stand-by state.
*
...
cy_en_sd_host_status_t Cy_SD_Host_SetRca (stc_SDHC_t *base, uint32_t *rca)
{
    ...
    ret = Cy_SD_Host_GetResponse(base, (uint32_t *)&response, false);
                                      ...
                                      uint32_t *ocrReg)
{
    ...
    *sdioFlag = *ocrReg & CY_SD_HOST_CMD5_IO_NUM_MASK;   
    ...
    {
        /* Software reset for the CMD and DAT lines. */
        Cy_SD_Host_SoftwareReset(base, CY_SD_HOST_RESET_DATALINE);
        Cy_SD_Host_SoftwareReset(base, CY_SD_HOST_RESET_CMD_LINE);
...
cy_en_sd_host_status_t  Cy_SD_Host_Enable(stc_SDHC_t *base)
{
    ...
    if (NULL != base)
    {
        ...
        base->CORE.unCLK_CTRL_R.u16Register = (uint16_t)_CLR_SET_FLD16U(base->CORE.unCLK_CTRL_R.u16Register, 
                                        SDHC_CORE_CLK_CTRL_R_INTERNAL_CLK_EN, 
                                                               1UL);

        while((true != _FLD2BOOL(SDHC_CORE_CLK_CTRL_R_INTERNAL_CLK_STABLE, base->CORE.unCLK_CTRL_R.u16Register))
              && (retry > 0UL))
        {
...
cy_en_sd_host_status_t Cy_SD_Host_SetSdClkDiv(stc_SDHC_t *base, uint16_t clkDiv)
{
    ...
    if (NULL != base)
    {
        temp.u16Register = base->CORE.unCLK_CTRL_R.u16Register;
        /* Clear the Clock divider bits */
        temp.u16Register = temp.u16Register & 0x003F;
        temp.u16Register = temp.u16Register | (( (clkDiv & 0x00FF)<<8) | ((clkDiv & 0x0300) >> 2));
        base->CORE.unCLK_CTRL_R.u16Register = temp.u16Register;

        /* Old Implementation */
        //temp.u16Register = (uint16_t)(_CLR_SET_FLD16U(base->CORE.unCLK_CTRL_R.u16Register, 
        ...
        //                                SDHC_CORE_CLK_CTRL_R_UPPER_FREQ_SEL_Msk)));
                                                  ...
                                                  cy_en_sd_host_bus_width_t width)
{
    ...
    if (NULL != base)
    {                        
        base->CORE.unHOST_CTRL1_R.u8Register = (uint8_t)(((uint32_t)base->CORE.unHOST_CTRL1_R.u8Register &
...
* \return \ref cy_en_sd_host_status_t
*
* \note The SD card power supply should be disabled when this function
*  returns CY_SD_HOST_ERROR_UNUSABLE_CARD.
*
* \note The dedicated io_volt_sel pin is used to change the regulator supplying  
* voltage to the VDDIO of the SD block in order to operate at 1.8V.
* The Cy_SD_Host_SelBusVoltage() function is not applicable when the user needs 
* to configure the custom IO pin to control (using the GPIO driver) the regulator
* supplying voltage. Before using the signal voltage switch with the custom IO pin,
* the card must be set to Ready state using the CMD0, CMD8 and ACMD41 commands sequence.   
* Refer to the 3.6.1 section of "Part A2 SD Host Controller Specification" 
* about the signal voltage switch procedure.
*
*******************************************************************************/
                                                ...
                                                cy_stc_sd_host_context_t *context)
{
    ...
    /* Get OCR (ACMD41). The voltage window = 0. */
    ret = Cy_SD_Host_OpsSdSendOpCond(base, &ocrReg, 0x0UL, context);
    ...
    if (CY_SD_HOST_SUCCESS == ret)
    {
        ...
        while (retry > 0UL)
        {
            ret = Cy_SD_Host_OpsSdSendOpCond(base, &ocrReg, cmdArgument, context);
            if (CY_SD_HOST_SUCCESS == ret)
            {
                break;
            }
                                               ...
                                               bool largeResponse)
{
        ...
        (NULL != responsePtr))
    {
        ...
        responseBaseAddr = (uint32_t*)&base->CORE.unRESP01_R.u32Register;
                                                   ...
                                                   cy_stc_sd_host_data_config_t const *dataConfig)
{
    ...
    if ((NULL != base) && (NULL != dataConfig) && (NULL != dataConfig->data))
    {
        ...
        dmaMode = _FLD2VAL(SDHC_CORE_HOST_CTRL1_R_DMA_SEL, base->CORE.unHOST_CTRL1_R.u8Register);
        
        if (((uint32_t)CY_SD_HOST_DMA_ADMA2_ADMA3 == dmaMode) && (dataConfig->enableDma))
        {
            /* ADMA3 Integrated Descriptor Address */
            base->CORE.unADMA_ID_LOW_R.u32Register = (uint32_t)dataConfig->data;
        }
        else
        {
            if (dataConfig->enableDma)
            {
                ...
                if ((uint32_t)CY_SD_HOST_DMA_SDMA == dmaMode)
                {
                    ...
                    base->CORE.unBLOCKSIZE_R.u16Register = _CLR_SET_FLD16U(base->CORE.unBLOCKSIZE_R.u16Register,
                                                  SDHC_CORE_BLOCKSIZE_R_SDMA_BUF_BDARY,
                                                  CY_SD_HOST_SDMA_BUF_BYTES_512K);

                    if (true == _FLD2BOOL(SDHC_CORE_HOST_CTRL2_R_HOST_VER4_ENABLE, base->CORE.unHOST_CTRL2_R.u16Register))
                    {
                        /* The data address. */
                        base->CORE.unADMA_SA_LOW_R.u32Register = (uint32_t)dataConfig->data;

                        /* Set the block count. */
                        base->CORE.unSDMASA_R.u32Register = dataConfig->numberOfBlock;
                    }
                    else
                    {
                        /* The data address. */
                        base->CORE.unSDMASA_R.u32Register = (uint32_t)dataConfig->data;
                    }
                }
                else
                {
            ...
            /* Set the block count. */
            base->CORE.unBLOCKCOUNT_R.u16Register = (uint16_t)dataConfig->numberOfBlock;
            ...
            base->CORE.unBGAP_CTRL_R.u8Register = (uint8_t)_CLR_SET_FLD8U(base->CORE.unBGAP_CTRL_R.u8Register, 
            ...
            base->CORE.unXFER_MODE_R.u16Register = transferMode;
        }
    }
...
*
*******************************************************************************/
void Cy_SD_Host_SoftwareReset(stc_SDHC_t *base, 
                              cy_en_sd_host_reset_t reset)
{
    switch (reset)
    {
        case CY_SD_HOST_RESET_DATALINE:
            base->CORE.unSW_RST_R.u8Register = (uint8_t)_VAL2FLD(SDHC_CORE_SW_RST_R_SW_RST_DAT, 1UL);

            while(false != _FLD2BOOL(SDHC_CORE_SW_RST_R_SW_RST_DAT, base->CORE.unSW_RST_R.u8Register))
            {
        ...
        case CY_SD_HOST_RESET_CMD_LINE:
            base->CORE.unSW_RST_R.u8Register = (uint8_t)_VAL2FLD(SDHC_CORE_SW_RST_R_SW_RST_CMD, 1UL);

            while(false != _FLD2BOOL(SDHC_CORE_SW_RST_R_SW_RST_CMD, base->CORE.unSW_RST_R.u8Register))
            {
                /* Wait until the reset completes. */                  
            }
        ...
        case CY_SD_HOST_RESET_ALL:
            base->CORE.unSW_RST_R.u8Register = (uint8_t)_VAL2FLD(SDHC_CORE_SW_RST_R_SW_RST_ALL, 1UL);

            while(false != _FLD2BOOL(SDHC_CORE_SW_RST_R_SW_RST_ALL, base->CORE.unSW_RST_R.u8Register))
            {
                /* Wait until the reset completes. */  
            }
...
uint32_t Cy_SD_Host_GetPresentState(stc_SDHC_t const *base)
{
    ...
    ret = base->CORE.unPSTATE_REG.u32Register;
