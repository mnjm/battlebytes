...
#include "crypto_sha2_test_vector.h"

/***********************************************************************************************************************
 * HMAC with SHA512
 ***********************************************************************************************************************/
void Run_SHA512(void)
{
    cy_stc_crypto_context_sha_t cryptoShaContext;

    /* Calculate HASH(s) */
    for (uint32_t i = 0; i < SHA512_NUMBER_OF_TEST; i++)
    {
        Cy_Crypto_Sha_Run
        (
            &cryptoCtx,
        ...
        CY_ASSERT(memcmp(&SHA512_hash[i], &SHA512_hash_ref[i], sizeof(SHA512_hash_ref[i])) == 0);
    }
    /* Then you can get the result from SHA512_hash */
}
