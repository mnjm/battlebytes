...
static void TestFor_DQUAD_MMIO_WRITE_Then_MMIO_DDR_READ_NON_BLOCKING(uint16_t rd_cmdId)
{
    ...
    for(uint32_t i_WriteOffsetAddrOfQuadDevice = 0; i_WriteOffsetAddrOfQuadDevice < (programSizeInByte / 2); i_WriteOffsetAddrOfQuadDevice+=DEVICE_PROGRAM_BUFFER_SIZE)
    {
        ...
        // Issue program
        Cy_SMIF_MMIO_Program_ExMem(SMIF_USED, D_QUAD_SLAVES, CY_SMIF_S25FL_ID_4QPP, writeStartAddress, (DEVICE_PROGRAM_BUFFER_SIZE*2), (uint8_t*)&swappedProgramData[writeOffsetAddrOfMCU], CY_SMIF_NON_BLOCKING, TEST_SPI_WRITE_LC, 0, &smifDeviceContext, &smifContext);
        ...
        CY_ASSERT_L1(Cy_SMIF_GetTxfrStatus(SMIF_USED, &smifContext) == CY_SMIF_SEND_CMPLT);
    ...
    Cy_SMIF_MMIO_Read_ExMem(SMIF_USED, D_QUAD_SLAVES, rd_cmdId, readAddress, readSizeInByte, (uint8_t*)readDataBuffer, TEST_SPI_READ_LC, CY_SMIF_S25FL_RW_NON_CONTINUOUS, CY_SMIF_NON_BLOCKING, &smifDeviceContext, &smifContext);
    ...
    CY_ASSERT_L1(Cy_SMIF_GetTxfrStatus(SMIF_USED, &smifContext) == CY_SMIF_REC_CMPLT);
