...
void irqFaultReport1Handler(void)
{
    ...
    if(status != CY_SYSFLT_NO_FAULT)
    {
        if(status == CY_SYSFLT_SRSS_MCWDT1) // MCWDT1 fault
        {
            ...
            {
                 Cy_GPIO_Inv(CY_LED5_PORT, CY_LED5_PIN);// subcounter 0 lower limit fault
            }
            else if(faultData & 0x00000002)
            {
                Cy_GPIO_Inv(CY_LED6_PORT, CY_LED6_PIN); // subcounter 0 upper limit fault
            }
            ...
            {
                Cy_GPIO_Inv(CY_LED5_PORT, CY_LED5_PIN); // subcounter 1 upper limit fault
            }
        }
    }
