...
#include "crypto_hmac_sha1_test_vector.h"

/***********************************************************************************************************************
 * HMAC with SHA1
 ***********************************************************************************************************************/
void Run_HMAC_SHA1(void)
{
    cy_stc_crypto_context_sha_t cryptoShaContext;

    /* Calculate HASH(s) */
    for (uint32_t i = 0; i < HMAC_SHA1_NUMBER_OF_TEST; i++)
    {
        Cy_Crypto_Hmac_Run
        (
            &cryptoCtx,
        ...
        CY_ASSERT(memcmp(&HMAC_SHA1_hash[i], &HMAC_SHA1_hash_ref[i], sizeof(HMAC_SHA1_hash_ref[i])) == 0);
    }
    /* Then you can get the result from HMAC_SHA1_hash */
}
