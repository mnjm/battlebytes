...
#define SECTOR_NO_USED_BY_CALIB         (255)           // Note the sector defined will be erased during delay line calibration
#define SMIF_DEVICE_USED                SMIF_DEVICE0
#define TARGET_CLK_FREQ_IN_MHZ          (166)           // Updates to the maximum supported speed (166MHz) on cluster platform
/**********************************************/
...
static bool DetectVolatileRegLantency(cy_en_smif_slave_select_t slave, cy_en_smif_semp_reg_latency_code_t* detectRegLatency)
{
    ...
    Cy_SMIF_SEMP_ReadId(SMIF_USED, slave, &readId, CY_SMIF_SEMP_REG_LATENCY0, CY_SPI_TRANSACTION_1S1S1S, sizeof(readId), &smifContext);
