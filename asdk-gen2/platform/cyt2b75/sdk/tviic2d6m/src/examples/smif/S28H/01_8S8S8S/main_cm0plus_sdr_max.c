...
int main(void)
{
    ...
    Cy_SMIF_DeviceSetRxCaptureDdr(SMIF_DEVICE0, CY_SMIF_RX_CAP_STYLE_DDR_OCTAL); // only for mode detection
    ...
    Cy_SMIF_DeviceSetRxCaptureDdr(SMIF_DEVICE1, CY_SMIF_RX_CAP_STYLE_DDR_OCTAL); // only for mode detection
    ...
    cy_en_smif_trx_type_t transMode = CY_SPI_TRANSACTION_8S8S8S; // From here on, this app will do 8S8S8S transaction.
    ...
    smifDev0Cfg.xipReadCmdId  = CY_SMIF_SEMP_RDID_8S8S8S_4ADR;
    smifDev0Cfg.xipWriteCmdId = CY_SMIF_SEMP_WTID_8S8S8S_4ADR;
