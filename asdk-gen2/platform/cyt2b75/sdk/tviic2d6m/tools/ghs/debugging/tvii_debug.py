...
#
# History:
# 2019-08-26 0.01 CHEY  Initial version for silicon
# 2019-10-25 0.02 CHEY  Move SRAM base address for GHS flash kernel to support new MCU revisions
#                       with relocated SROM private RAM
# 2019-12-09 0.03 CHEY  - Migrated to 'argparser' Python module and introducing new optional parameter for script
...
# 2020-01-16 0.04 CHEY  Added API for executing DAP system calls -> added output of current life-cycle state
# 2020-01-31 0.05 CHEY  Changed trace setup and some related parts (see Readme.txt of SDL for version/tool information)
# 2020-02-07 0.06 CHEY  Adding print of SFLASH data revision, split printing of target info in 2 lines now
# 2020-07-10 0.07 CHEY  - Improve IsTpiuTraceProbe() for certain cases
...
# 2020-10-06 0.08 CHEY  Clear some control bits in DHCSR register before triggering SYSRESET to fix problem
# 2020-10-20 0.09 CHEY  Check for "Could not detect flash device" message and print potential solution
# 2021-07-15 0.10 CHEY  Add offchip trace config for 327-BGA board (MCU Rev. B) and make it the default
# 2021-09-22 0.11 CHEY  Add support for SFLASH user row programming
# 2022-01-27 0.12 CHEY  Remove MCU rev_a trace pin configuration support
# ***********************************************************************************************/#
...
CPU_BOARD_TYPE = 'CPU_BOARD_327_BGA'  # Set 'CPU_BOARD_327_BGA', no other CPU board available at the moment
...
TRACE_CLK_DIV_NR = 8  # Divider instance number. Index is 0-based, default value is the last "8.0" type divider of this device
...
TRACE_PIN_CONFIG = {
                        'CPU_BOARD_327_BGA' : {
                                                'CLK'    : { 'PORT' : 18, 'PIN' : 0, 'HSIOM_SEL' : 11 },
                                                'DATA_0' : { 'PORT' : 18, 'PIN' : 1, 'HSIOM_SEL' : 11 },
                                                'DATA_1' : { 'PORT' : 18, 'PIN' : 2, 'HSIOM_SEL' : 11 },
                                                'DATA_2' : { 'PORT' : 18, 'PIN' : 3, 'HSIOM_SEL' : 11 },
                                                'DATA_3' : { 'PORT' : 18, 'PIN' : 4, 'HSIOM_SEL' : 11 },
                                               #--------- For 8-bit trace support Mictor connector J15 needs to be mounted on eval board!
                                               #'DATA_4' : { 'PORT' : 16, 'PIN' : 3, 'HSIOM_SEL' : 11 },
                                               #'DATA_5' : { 'PORT' : 16, 'PIN' : 4, 'HSIOM_SEL' : 11 },
                                               #'DATA_6' : { 'PORT' : 16, 'PIN' : 5, 'HSIOM_SEL' : 11 },
                                               #'DATA_7' : { 'PORT' : 16, 'PIN' : 6, 'HSIOM_SEL' : 11 },                                                
                                            }                                            
                    }                      
...
CODE_FLASH_SIZE_BYTES                      = 6336 * 1024
...
if EXECUTION_MODE == "ERASE":
    if IS_PSVP is True:
        # only 1MB of non-contiguous flash memory in PSVP
        EraseCodeFlash(0, 512*1024)
        EraseCodeFlash(0x002f8000, 512*1024)
    else:
        EraseCodeFlash() 
    ResetMcuViaCm0AircrSysResetReqAndHalt()
