...
#define TERMINAL_UART           CY_USB_SCB_UART_TYPE
...
void UartInterruptHandler(void)
{
    ...
    uint32_t maskedRx = Cy_SCB_GetRxInterruptStatusMasked(CY_USB_SCB_UART_TYPE);
    uint32_t maskedTx = Cy_SCB_GetTxInterruptStatusMasked(CY_USB_SCB_UART_TYPE);

    if((maskedRx & SCB_INTR_RX_TRIGGER_Msk) != 0UL)
    {
        while(0UL == Cy_SCB_UART_GetArray(CY_USB_SCB_UART_TYPE, &inData, 1));
        ...
        else if(inData >= '1' && inData <= '5')
        {
            ...
            {
                Cy_SCB_UART_PutArray(CY_USB_SCB_UART_TYPE, &inData, 1);
                Cy_SCB_UART_PutArray(CY_USB_SCB_UART_TYPE, "\b", 1);
            }
            prevData = inData;
        }
    }

    Cy_SCB_ClearRxInterrupt(CY_USB_SCB_UART_TYPE, maskedRx);
    Cy_SCB_ClearTxInterrupt(CY_USB_SCB_UART_TYPE, maskedTx);
}
