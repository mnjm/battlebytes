...
void LIN0_IntHandler(void)
{
    ...
    if ((maskStatus & CY_LIN_INTR_ALL_ERROR_MASK_SLAVE) != 0u)
    {
        ...
        /* Re enable header RX */
        lin_state = LIN_STATE_RX_HEADER;
        Cy_Lin_SetInterruptMask(CY_LINCH0_TYPE, CY_LIN_INTR_RX_HEADER_DONE | CY_LIN_INTR_RX_RESPONSE_DONE | CY_LIN_INTR_ALL_ERROR_MASK_SLAVE);
        Cy_Lin_SetCmd(CY_LINCH0_TYPE, CY_LIN_CMD_RX_HEADER_RX_RESPONSE);
    }
    else
    {
        ...
        case LIN_STATE_RX_HEADER:
            ...
            Cy_Lin_GetHeader(CY_LINCH0_TYPE, &id, &parity);
            /* Analyze ID */
            ...
            if (acceptedId)
            {
                ...
                Cy_Lin_SetDataLength(CY_LINCH0_TYPE, msgContext[currentMsgIdx].dataLength);
                Cy_Lin_SetChecksumType(CY_LINCH0_TYPE, msgContext[currentMsgIdx].checksumType);
                if (msgContext[currentMsgIdx].responseDirection == LIN_TX_RESPONSE)
                {
                    Cy_Lin_SetInterruptMask(CY_LINCH0_TYPE, CY_LIN_INTR_TX_RESPONSE_DONE | CY_LIN_INTR_RX_RESPONSE_DONE | CY_LIN_INTR_ALL_ERROR_MASK_SLAVE);
                    Cy_Lin_WriteData(CY_LINCH0_TYPE, msgContext[currentMsgIdx].dataBuffer, msgContext[currentMsgIdx].dataLength);
                    lin_state = LIN_STATE_TX_RESPONSE;
                    Cy_Lin_SetCmd(CY_LINCH0_TYPE, CY_LIN_CMD_TX_RESPONSE);
                }
                else
                {
        ...
        case LIN_STATE_RX_RESPONSE:
            ...
            while(1)
            {
                apiResponse = Cy_Lin_ReadData(CY_LINCH0_TYPE, msgContext[currentMsgIdx].dataBuffer);
