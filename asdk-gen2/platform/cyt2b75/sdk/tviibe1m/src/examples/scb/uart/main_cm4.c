...
int main(void)
{
    ...
    for(;;)
    {
        ...
        uint32_t num = Cy_SCB_UART_GetNumInRingBuffer(CY_USB_SCB_UART_TYPE, &g_stc_uart_context);
        if (num != 0) {
            Cy_SCB_UART_Receive(CY_USB_SCB_UART_TYPE, &g_uart_user_buf[0], num, &g_stc_uart_context);
            Cy_SCB_UART_Transmit(CY_USB_SCB_UART_TYPE, &g_uart_user_buf[0], num, &g_stc_uart_context);
        }
#endif
    }
}
