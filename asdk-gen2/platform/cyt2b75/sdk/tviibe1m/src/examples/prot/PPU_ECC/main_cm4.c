...
static uint128_t MakeWordForPpuEcc(const stc_ppu_all_attribute_t* attValue, uint32_t ppuWordAddr);
...
int main(void)
{
    ...
    *att.pu32[3] = gTestAttValue.att[3];
    ...
    uint128_t word = MakeWordForPpuEcc(&gTestAttValue ,wordAddr);
...
static uint128_t MakeWordForPpuEcc(const stc_ppu_all_attribute_t* attValue, uint32_t ppuWordAddr)
{
    ...
    word.u64[1] |= (ppuWordAddr & 0x000003FFul) << (75ul - 64ul);
