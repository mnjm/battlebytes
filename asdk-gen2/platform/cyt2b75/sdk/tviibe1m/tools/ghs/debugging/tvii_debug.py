...
#
# History:
# 2018-01-10 0.01 CHEY  Initial version for Traveo II family
# 2018-01-24 0.02 CHEY  Fixed problem that M4 executable sometimes does not appear as loaded after
#                       flashing. Creating aliases.
# 2018-01-24 0.03 CHEY  Removed obsolete print messages
# 2018-02-02 0.04 CHEY  Removed redundant/duplicate "halt CM0+" in DoReset
# 2018-02-08 0.05 DENA/ Fixed problem that CM4 would halt in BootRom. Separate Read/Write procedures
#                 CHEY  for CM0+ and CM4.
# 2018-02-25 0.06 CHEY  Changed connection mechanism for PSVP/silicon
# 2018-02-26 0.07 CHEY  Flash programming workaround, tested on silicon
# 2018-02-28 0.08 CHEY  Changes in flash programming to make it more robust
# 2018-02-28 0.09 CHEY  Automatically reduces JTAG frequency for long cable
# 2018-03-22 0.10 CHEY  Fix detection of CY_USE_PSVP define.
# 2018-04-20 0.11 CHEY  Major changes (tested with TVII-B-E-1M Flash Boot ES10, probe FW/driver 5.6.4
#                       and GHS patch #12241 for TVII flash support):
#                       - PSVP support removed
#                       - Debug clock reduction for long cables removed, instead bypass the PMIC
#                         with 3.3V or make a local modification of clock value in tviibe1m.ghpcfg,
#                         or use devices in NORMAL lifecycle mode that will configure TTL inputs.
#                       - Major refactoring of functions
#                       - Script now also configures a breakpoint for M4.
#                       - Adding "ERASE" mode for script
# 2018-04-25 0.12 CHEY  Fixed DoReset() for CM0+ only operation
# 2018-06-08 0.13 CHEY  Fixing some issues when connecting (read command not successful)
# 2018-06-20 0.14 CHEY  Adding IS_PSVP_WITHOUT_FLASH_BOOT_FW config option to make the scripts
#                       working with PSVP again
# 2018-07-18 0.15 CHEY  Trace support added
# 2018-08-21 0.16 CHEY  Fixed SuperTraceProbe detection. Fixed space issue in path names.
#                       Toolbar buttons for resume/halt all cores added. 'IS_PSVP_WITHOUT_FLASH_BOOT_FW'
#                       split into 2 separate configuration settings. CM0 vector table base is detected
#                       for patching of RAM build entry. Rename of and slight changes to former
#                       'DoReset' function. New trace clock divider. Other minor changes.
# 2018-08-27 0.17 CHEY  Added selection of connection type (USB or Ethernet)
# 2018-11-06 0.18 CHEY  Some refactoring and merged all possible changes from TVII-B-H-8M v0.12 script
# 2018-11-22 0.19 CHEY  Fixed issue that CM0+ does not halt at configured user breakpoint in case of
#                       of Flash build. Added support for routing printf() to debugger "I/O" tab.
# 2018-11-22 0.20 CHEY  Added clearing of PRIMASK before flash programming (may help for some flash issues)
# 2019-02-26 0.21 CHEY  - Added automatic determination of CM0_RESET_VECTOR_CATCH_IS_WORKING
...
# 2019-03-15 0.22 CHEY  Updated and tested external trace setup. Fixed issue in CheckProbeSettings()
# 2019-10-25 0.23 CHEY  Move SRAM base address for GHS flash kernel to support new MCU revisions
#                       with relocated SROM private RAM
# 2019-12-09 0.24 CHEY  - Migrated to 'argparser' Python module and introducing new optional parameter for script
...
# 2020-01-16 0.25 CHEY  Added API for executing DAP system calls -> added output of current life-cycle state
# 2020-01-31 0.26 CHEY  Changed trace setup and some related parts based on new CM7 trace setup
# 2020-02-07 0.27 CHEY  Adding print of SFLASH data revision, split printing of target info in 2 lines now
# 2020-07-10 0.28 CHEY  - Improve IsTpiuTraceProbe() for certain cases
...
# 2020-10-06 0.29 CHEY  Clear some control bits in DHCSR register before triggering SYSRESET to fix problem
# 2020-10-20 0.30 CHEY  Check for "Could not detect flash device" message and print potential solution
# 2021-09-22 0.31 CHEY  Add support for SFLASH user row programming
# ***********************************************************************************************/#
...
# Set the label/address (see GHS manual for supported formats) where the
# debugger will put a breakpoint for the CM4 process. Must be a string.
# Note: This breakpoint can typically only be hit after CM4 has been enabled by CM0+.
...
CPU_BOARD_TYPE = 'CPU_BOARD_176_QFP_REV_C'  # Set 'CPU_BOARD_176_QFP_REV_A' or 'CPU_BOARD_176_QFP_REV_C'
...
TRACE_CLK_DIV_NR = 31 # Divider instance number. Index is 0-based, default value is the last "8.0" type divider of this device
...
TRACE_PIN_CONFIG = {
                        ...
                        'CPU_BOARD_176_QFP_REV_C' : {
                                                      'CLK'    : { 'PORT' : 18, 'PIN' : 3, 'HSIOM_SEL' : 27 },
                                                      'DATA_0' : { 'PORT' : 22, 'PIN' : 0, 'HSIOM_SEL' : 27 },
                                                      'DATA_1' : { 'PORT' : 22, 'PIN' : 1, 'HSIOM_SEL' : 27 },
                                                      'DATA_2' : { 'PORT' : 22, 'PIN' : 2, 'HSIOM_SEL' : 27 },
                                                      'DATA_3' : { 'PORT' : 22, 'PIN' : 3, 'HSIOM_SEL' : 27 },
                                                  }
...
VALUE__TRC_CSTF_CSTFCTL__ETM_CM4   = 0x00000301
...
REG__PERI_CLOCK_CTL_0        = 0x40000c00                          # Control register for PCLK_CPUSS_CLOCK_TRACE_IN
REG__PERI_DIV8_CTL_N         = 0x40001000 + (TRACE_CLK_DIV_NR * 4) # Control register for n-th 8-bit divider
REG__PERI_DIV_CMD            = 0x40000400
VALUE__PERI_CLOCK_CTL_0      = (TRACE_CLK_DIV_NR & 0x1f)           # Selects the user chosen 8-bit divider for PCLK_CPUSS_CLOCK_TRACE_IN
VALUE__PERI_DIV8_CTL_N       = ((TRACE_CLK_DIV - 1) << 8)          # Set divider value according to user configuration
VALUE__PERI_DIV_CMD          = 0x83ff0000 | TRACE_CLK_DIV_NR       # Enable the n-th 8-bit divider (no phase alignment to other divider)
...
CODE_FLASH_SIZE_BYTES                      = 1088 * 1024
...
CM4_INCLUDED_IN_GHSMC_FILE = True
...
# Executes a debugger command routed via CM4
def ExecDebugCmdViaCm4(cmd_string, overwrite_print=False):
    ExecDebugCmd("m4 " + cmd_string, overwrite_print)
...
# Reads memory via CM4, returns None on read error
def ReadViaCm4(address, width_bytes=4):
    return _ReadInternal(address, width_bytes, "m4")
...
# Writes memory via CM4, returns True on success and False on error (please note: no verification of written data by read-after-write!)
def WriteViaCm4(address, value, width_bytes=4):
    return _WriteInternal(address, value, width_bytes, "m4")
...
def EvaluateAliasesInGhsmcFile():
    ...
    global CM4_INCLUDED_IN_GHSMC_FILE
    ...
    ExecDebugCmd("m4 echo")
    if (GetDebugCmdOutput().lower().find("components") != -1):
        CM4_INCLUDED_IN_GHSMC_FILE = False
...
def SetUserBreakpoints():
    if SKIP_BREAKPOINTS is False:
        ...
        # This is a prerequisite for routing printf() etc. to the GHS debugger "I/O" tab (the other one is "target syscalls on")    
        bp_hit_cmd = " { target just_did_rom_to_ram_copy }"
        ...
        ExecDebugCmdViaCm0(bp_cmd + BREAKPOINT_LABEL_M0 + bp_hit_cmd)
        if CM4_INCLUDED_IN_GHSMC_FILE is True:
            ExecDebugCmdViaCm4(bp_cmd + BREAKPOINT_LABEL_M4 + bp_hit_cmd)
...
def ConfigureCoreSightTracePaths():
    ...
    if CM4_INCLUDED_IN_GHSMC_FILE is False:
        return False        
    # Setup all necessary paths as per the user configuration

    ExecDebugCmd("target coresight_path add 2 [CM4] -> 4.0 [CSTF: ETM of CM4] -> " + ("5 [ETB]" if USE_TPIU_OFFCHIP_TRACE is False else "6 [TPIU]"))
    return True
    
# Configures trace related settings
def SetupTrace():
    # GHS does not support the Micro Trace Buffer (MTB) of CM0+, so there is only something to do if CM4 shall be debugged as well
    if CM4_INCLUDED_IN_GHSMC_FILE is False:
        return
    ...
    ExecDebugCmdViaCm4("trace disable; trace abort")
    ...
    if USE_TPIU_OFFCHIP_TRACE is True:
        ...
        WriteViaCm0(REG__PERI_DIV8_CTL_N,  VALUE__PERI_DIV8_CTL_N)
        WriteViaCm0(REG__PERI_CLOCK_CTL_0, VALUE__PERI_CLOCK_CTL_0)
        WriteViaCm0(REG__PERI_DIV_CMD,     VALUE__PERI_DIV_CMD)
    ...
    if USE_TPIU_OFFCHIP_TRACE is False:
        ...
        WriteViaCm4(REG__TPIU_ITCTRL,    0x00000001) # Enable Integration Mode
        WriteViaCm4(REG__TPIU_ITATBCTR2, 0x00000001) # Set ATREADYS
        
    # Enable CoreSight Trace Funnel slave ports for the ETM of CM4, ITM not supported by GHS(?)
    WriteViaCm4(REG__TRC_CSTF_CSTFCTL, VALUE__TRC_CSTF_CSTFCTL__ETM_CM4)
    
    # (Re-)enable trace
    ExecDebugCmdViaCm4("trace enable")
...
def ProgramCodeFlash():
    ...
    if CM4_INCLUDED_IN_GHSMC_FILE is True:
        ResetAndPrepareFlashOperation()
        ExecDebugCmdViaCm4("map -modules") # Will print full path to CM4 .elf file
        ELF_PATH = "\"" + GetDebugCmdOutput() + "\""
        print("Programming CM4 application image")
        # GHS flash kernel must be run on CM0+ at the moment
        ExecDebugCmdViaCm0("flash burn -baseaddress=" + ConvertToHex(CODE_FLASH_BASE_ADDRESS) + sflash_permission_string +
                                     " -rambase="     + ConvertToHex(SRAM_BASE_ADDRESS_FOR_GHS_FLASHKERNEL) +
                                     " -memrequire=64" +
...
def VerifyApplication():
    ...
    if CM4_INCLUDED_IN_GHSMC_FILE is True:
        ExecDebugCmdViaCm4(verify_cmd, True)
...
def DoReset():
    ...
    if CM4_INCLUDED_IN_GHSMC_FILE is True:
        ChangeResetVectorCatch("m4", False)
    SetupTrace()
...
ExecDebugCmd("alias m4 route cm4")
