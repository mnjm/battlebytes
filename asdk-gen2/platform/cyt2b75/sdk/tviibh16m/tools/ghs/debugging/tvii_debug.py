...
#
# History:
# 2022-05-30 0.xx CHEY  >>> no history logging for early releases <<<
# ***********************************************************************************************/#
...
IS_PSVP = True  # Set True or False
...
# Set the label/address (see GHS manual for supported formats) where the
# debugger will put a breakpoint for the CM7_0/CM7_1/CM7_2/CM7_3 process. Must be a string.
# Note: This breakpoint can typically only be hit after CM7_0/CM7_1/CM7_2/CM7_3 has been enabled by CM0+.
BREAKPOINT_LABEL_M7_0 = "main"
BREAKPOINT_LABEL_M7_1 = "main"
BREAKPOINT_LABEL_M7_2 = "main"
BREAKPOINT_LABEL_M7_3 = "main"
...
TRACE_CM7_2 = False  # Set True or False
TRACE_CM7_3 = False  # Set True or False
...
VALUE__TRC_CSTF_CSTFCTL__ETM_CM7_2 = 0x00000320  # Enables funnel input for ETM trace of CM7_2 (slave port #5)
VALUE__TRC_CSTF_CSTFCTL__ETM_CM7_3 = 0x00000380  # Enables funnel input for ETM trace of CM7_3 (slave port #7)
...
REG__CPUSS_CM7_2_PWR_CTL           = 0x40201220
REG__CPUSS_CM7_3_PWR_CTL           = 0x40201230
...
REG__CPUSS_CM7_2_CTL               = 0x4020080c
REG__CPUSS_CM7_3_CTL               = 0x40200c0c
...
REG__FLASHC1_FM_CTL_ECT_MAIN_FLASH_SAFETY  = 0x4025f400
REG__FLASHC1_FM_CTL_ECT_WORK_FLASH_SAFETY  = 0x4025f500
...
CODE_FLASH1_BASE_ADDRESS                   = 0x18000000
CODE_FLASH_SIZE_BYTES                      = 8384 * 1024 # size of single macro
...
CM7_2_INCLUDED_IN_GHSMC_FILE                   = True
CM7_3_INCLUDED_IN_GHSMC_FILE                   = True
CM7_0_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED = None
CM7_1_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED = None
CM7_2_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED = None
CM7_3_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED = None
...
# Executes a debugger command routed via CM7_2
def ExecDebugCmdViaCm7_2(cmd_string, overwrite_print=False):
    ExecDebugCmd("m7_2 " + cmd_string, overwrite_print)

# Executes a debugger command routed via CM7_3
def ExecDebugCmdViaCm7_3(cmd_string, overwrite_print=False):
    ExecDebugCmd("m7_3 " + cmd_string, overwrite_print)
...
# Reads memory via CM7_2, returns None on read error
def ReadViaCm7_2(address, width_bytes=4):
    return _ReadInternal(address, width_bytes, "m7_2")

# Reads memory via CM7_3, returns None on read error
def ReadViaCm7_3(address, width_bytes=4):
    return _ReadInternal(address, width_bytes, "m7_3")
...
# Writes memory via CM7_2, returns True on success and False on error (please note: no verification of written data by read-after-write!)
def WriteViaCm7_2(address, value, width_bytes=4):
    return _WriteInternal(address, value, width_bytes, "m7_2")

# Writes memory via CM7_3, returns True on success and False on error (please note: no verification of written data by read-after-write!)
def WriteViaCm7_3(address, value, width_bytes=4):
    return _WriteInternal(address, value, width_bytes, "m7_3")
...
def IsApplicationLinkedForFlash():
    ...
    if ( ((main_address >= CODE_FLASH_BASE_ADDRESS)  and (main_address < (CODE_FLASH_BASE_ADDRESS  + CODE_FLASH_SIZE_BYTES))) or
         ((main_address >= CODE_FLASH1_BASE_ADDRESS) and (main_address < (CODE_FLASH1_BASE_ADDRESS + CODE_FLASH_SIZE_BYTES)))    ):
        return True
    else:
        return False
...
def EvaluateAliasesInGhsmcFile():
    ...
    global CM7_2_INCLUDED_IN_GHSMC_FILE
    global CM7_3_INCLUDED_IN_GHSMC_FILE
    ...
    ExecDebugCmd("m7_2 echo")
    if (GetDebugCmdOutput().lower().find("components") != -1):
        CM7_2_INCLUDED_IN_GHSMC_FILE = False
    ExecDebugCmd("m7_3 echo")
    if (GetDebugCmdOutput().lower().find("components") != -1):
        CM7_3_INCLUDED_IN_GHSMC_FILE = False
...
def SetUserBreakpoints():
    if SKIP_BREAKPOINTS is False:
        ...
        if CM7_2_INCLUDED_IN_GHSMC_FILE is True:
            ExecDebugCmdViaCm7_2(bp_cmd + BREAKPOINT_LABEL_M7_2 + bp_hit_cmd_cm7)
        if CM7_3_INCLUDED_IN_GHSMC_FILE is True:
            ExecDebugCmdViaCm7_3(bp_cmd + BREAKPOINT_LABEL_M7_3 + bp_hit_cmd_cm7)
...
def ConfigureCoreSightTracePaths():
    ...
    if ((CM7_0_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED is False) and 
        (CM7_1_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED is False) and
        (CM7_2_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED is False) and
        (CM7_3_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED is False)):
        return False        
    ...
    if CM7_2_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED:
        ExecDebugCmd("target coresight_path add 4 [CM7_2] -> 6.5 [CSTF: ETM of CM7_2] -> " + ("7 [ETB]" if USE_TPIU_OFFCHIP_TRACE is False else "8 [TPIU]"))
    if CM7_3_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED:
        ExecDebugCmd("target coresight_path add 5 [CM7_3] -> 6.7 [CSTF: ETM of CM7_3] -> " + ("7 [ETB]" if USE_TPIU_OFFCHIP_TRACE is False else "8 [TPIU]"))    
    return True
...
def SetupTrace():
    ...
    if CM7_2_INCLUDED_IN_GHSMC_FILE is True:
        ExecDebugCmdViaCm7_2("trace disable; trace abort")
    if CM7_3_INCLUDED_IN_GHSMC_FILE is True:
        ExecDebugCmdViaCm7_3("trace disable; trace abort")
    ...
    #       not included in .ghsmc file (e.g. cm0plus_with_cm7_1.ghsmc). Therefore, at some places in the code below, the more generic low level
    ...
    cstf_ctl_value = (VALUE__TRC_CSTF_CSTFCTL__ETM_CM7_0 if CM7_0_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED is True else 0) | \
                     ...
                     (VALUE__TRC_CSTF_CSTFCTL__ETM_CM7_2 if CM7_2_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED is True else 0) | \
                     (VALUE__TRC_CSTF_CSTFCTL__ETM_CM7_3 if CM7_3_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED is True else 0)
    ...
    if CM7_2_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED is True:
        ExecDebugCmdViaCm7_2("trace enable")
    if CM7_3_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED is True:
        ExecDebugCmdViaCm7_3("trace enable")    
...
def EnableFlashWriting():
    ...
    WriteViaCm0(REG__FLASHC1_FM_CTL_ECT_MAIN_FLASH_SAFETY, VALUE__FLASHC_FM_CTL_ECT_MAIN_FLASH_SAFETY)
    WriteViaCm0(REG__FLASHC1_FM_CTL_ECT_WORK_FLASH_SAFETY, VALUE__FLASHC_FM_CTL_ECT_WORK_FLASH_SAFETY)
...
# Erase code flash macro from given offset and with given size (default is whole code flash macro)
def EraseCodeFlash(offset=0, size=CODE_FLASH_SIZE_BYTES, macro=0):
    ...
    with tempfile.NamedTemporaryFile(delete = False) as temp_file_for_erase:
        try:
            ...
            print("Erasing CodeFlash macro #" + str(macro) + " from offset " + ConvertToHex(offset) + " (" + str(offset/1024) + " KB)" + " with size " + ConvertToHex(size) + " (" + str(size/1024) + " KB). This may take a while...")
            flash_macro_base_address = CODE_FLASH_BASE_ADDRESS if macro == 0 else CODE_FLASH1_BASE_ADDRESS
            # GHS flash kernel must be run on CM0+ at the moment
            ExecDebugCmdViaCm0("flash burn -baseaddress="  + ConvertToHex(flash_macro_base_address) +
...
def ProgramCodeFlash():
    ...
    if CM7_2_INCLUDED_IN_GHSMC_FILE is True:
        ResetAndPrepareFlashOperation()
        ExecDebugCmdViaCm7_2("map -modules") # Will print full path to CM7_2 .elf file
        ELF_PATH = "\"" + GetDebugCmdOutput() + "\""
        print("Programming CM7_2 application image")
        # GHS flash kernel must be run on CM0+ at the moment
        ExecDebugCmdViaCm0("flash burn -baseaddress=" + ConvertToHex(CODE_FLASH_BASE_ADDRESS) + sflash_permission_string +
                                     " -rambase="     + ConvertToHex(SRAM_BASE_ADDRESS_FOR_GHS_FLASHKERNEL) +
                                     " -memrequire=64" +
    ...
    if CM7_3_INCLUDED_IN_GHSMC_FILE is True:
        ResetAndPrepareFlashOperation()
        ExecDebugCmdViaCm7_3("map -modules") # Will print full path to CM7_3 .elf file
        ELF_PATH = "\"" + GetDebugCmdOutput() + "\""
        print("Programming CM7_3 application image")
        # GHS flash kernel must be run on CM0+ at the moment
        ExecDebugCmdViaCm0("flash burn -baseaddress=" + ConvertToHex(CODE_FLASH_BASE_ADDRESS) + sflash_permission_string +
                                     " -rambase="     + ConvertToHex(SRAM_BASE_ADDRESS_FOR_GHS_FLASHKERNEL) +
                                     " -memrequire=64" +
...
def VerifyApplication():
    ...
    if CM7_2_INCLUDED_IN_GHSMC_FILE is True:
        ExecDebugCmdViaCm7_2(verify_cmd, True)
    if CM7_3_INCLUDED_IN_GHSMC_FILE is True:
        ExecDebugCmdViaCm7_3(verify_cmd, True)
...
def DoReset():
    ...
    if CM7_2_INCLUDED_IN_GHSMC_FILE is True:
        pass # ChangeResetVectorCatch("m7_2", False)
    if CM7_3_INCLUDED_IN_GHSMC_FILE is True:
        pass # ChangeResetVectorCatch("m7_3", False)
    SetupTrace()
    EnsureTargetIsStoppedAtUserBreakpoint()

def EnableCm7Access():
    WriteViaCm0(REG__SRSS_CLK_ROOT_SELECT1, VALUE__SRSS_CLK_ROOT_SELECT) # enable CLK_HF1 (CM7_0, CM7_1, CM7_2, CM7_3)
    ...
    WriteViaCm0(REG__CPUSS_CM7_2_CTL, ReadViaCm0(REG__CPUSS_CM7_2_CTL) | 0x00800a00) # Set init values for "DTCM Enable and RMW", enable DTCM AHB Slave Port
    WriteViaCm0(REG__CPUSS_CM7_3_CTL, ReadViaCm0(REG__CPUSS_CM7_3_CTL) | 0x00800a00) # Set init values for "DTCM Enable and RMW", enable DTCM AHB Slave Port
    ...
    WriteViaCm0(REG__CPUSS_CM7_2_PWR_CTL, VALUE__CPUSS_CM7_x_PWR_CTL__ENABLE)
    WriteViaCm0(REG__CPUSS_CM7_3_PWR_CTL, VALUE__CPUSS_CM7_x_PWR_CTL__ENABLE)
    ...
    WriteViaCm0(REG__CPUSS_CM7_2_PWR_CTL, VALUE__CPUSS_CM7_x_PWR_CTL__RESET)
    WriteViaCm0(REG__CPUSS_CM7_3_PWR_CTL, VALUE__CPUSS_CM7_x_PWR_CTL__RESET)
    ...
    WriteViaCm0(REG__CPUSS_CM7_2_CTL, ReadViaCm0(REG__CPUSS_CM7_2_CTL) & (~(MASK__CPUSS_CM7_x_CTL_CPU_WAIT))) # Clear CPU_WAIT, let CM7 execute the endless "WFI" loop in SROM
    WriteViaCm0(REG__CPUSS_CM7_3_CTL, ReadViaCm0(REG__CPUSS_CM7_3_CTL) & (~(MASK__CPUSS_CM7_x_CTL_CPU_WAIT))) # Clear CPU_WAIT, let CM7 execute the endless "WFI" loop in SROM
    ...
    ExecDebugCmdViaCm7_2("target rw ghs_usable 1")
    ExecDebugCmdViaCm7_3("target rw ghs_usable 1")
...
ExecDebugCmd("alias m7_2 route cm7_2")
ExecDebugCmd("alias m7_3 route cm7_3")
...
CM7_2_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED = True if ((CM7_2_INCLUDED_IN_GHSMC_FILE is True) and (TRACE_CM7_2 is True)) else False
CM7_3_INCLUDED_IN_GHSMC_FILE_AND_TRACE_ENABLED = True if ((CM7_3_INCLUDED_IN_GHSMC_FILE is True) and (TRACE_CM7_3 is True)) else False
