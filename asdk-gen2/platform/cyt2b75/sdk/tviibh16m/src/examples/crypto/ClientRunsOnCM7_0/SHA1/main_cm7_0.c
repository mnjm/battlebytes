...
#include "crypto_sha1_test_vector.h"
...
void Run_SHA1(void)
{
    ...
    /* Calculate HASH(s) */
    for (uint32_t i = 0; i < SHA1_NUMBER_OF_TEST; i++)
    {
        Cy_Crypto_Sha_Run
        (
            &cryptoCtx,
        ...
        CY_ASSERT(memcmp(&SHA1_hash[i], &SHA1_hash_ref[i], sizeof(SHA1_hash_ref[i])) == 0);
    }
    /* Then you can get the result from SHA1_hash */
}
