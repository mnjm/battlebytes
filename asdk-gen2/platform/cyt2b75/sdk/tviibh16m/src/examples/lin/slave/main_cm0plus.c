...
{
    {0x01, LIN_TX_RESPONSE, LinChecksumTypeExtended, 8u,}, // From master, 8 bytes as a subscriber
    {0x02, LIN_RX_RESPONSE, LinChecksumTypeExtended, 8u,}, // From master, send 8 bytes as a publisher
    {0x10, LIN_TX_RESPONSE, LinChecksumTypeExtended, 1u,}, // From master, 1 byte as a subscriber
    {0x11, LIN_RX_RESPONSE, LinChecksumTypeExtended, 1u,}, // From master, send 1 byte as a publisher
};
...
#if (CY_USE_PSVP == 1) 
    Cy_SysClk_PeriphSetDivider(Cy_SysClk_GetClockGroup(CY_LINCH_PCLK), CY_SYSCLK_DIV_16_BIT, 0ul, 63ul);        // 20MHz/63/16 = 19,841Hz
#else
    Cy_SysClk_PeriphSetDivider(Cy_SysClk_GetClockGroup(CY_LINCH_PCLK), CY_SYSCLK_DIV_16_BIT, 0ul, 260ul);       // 80MHz/260/16 = 19,230Hz
#endif
