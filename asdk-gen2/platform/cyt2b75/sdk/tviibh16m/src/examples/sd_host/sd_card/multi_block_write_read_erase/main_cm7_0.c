...
int main(void)
{
    ...
    //for(uint16_t i=0; i<sizeof(u8TxBuff); i++, u8TxBuff[i]=0);
    u8ch = gu8StartCh;
    ...
    for ( i = 0; i < 4u; i++ )
    {
        ...
        //for(uint16_t i=0; i<CY_SD_HOST_BLOCK_SIZE*SECTORNUM; i++, u8TxBuff[i]=u8ch);
        u8ch++;
    ...
    for ( i = 0; i < 4u; i++ )
    {
        ...
        //for(uint16_t i=0; i<sizeof(u8RxBuff); i++, u8RxBuff[i]=0);
    ...
    enRet = Cy_SD_Host_Erase((stc_SDHC_t *)CY_SDHC_TYPE, 1,
    ...
    enRet = Cy_SD_Host_PollCmdComplete((stc_SDHC_t *)CY_SDHC_TYPE);
    ...
    while (i-- > 0u)
    {
        cardStatus = Cy_SD_Host_GetCardStatus((stc_SDHC_t *)CY_SDHC_TYPE, &SDIF0_context);
    ...
    //read back & check.
    memset(u8RxBuff, 0x0, sizeof(u8RxBuff));
    //for(uint16_t i=0; i<sizeof(u8RxBuff); i++, u8RxBuff[i]=0x0);
    memset(u8TxBuff, 0x5, sizeof(u8TxBuff));
    //for(uint16_t i=0; i<sizeof(u8TxBuff); i++, u8TxBuff[i]=0x5);
