...
int main(void)
{
    ...
    un_cy_smif_s25fs_CFR3_t readConfigReg3 = {0};
    Cy_SMIF_S25FS_ReadAnyRegister(SMIF_USED,
                                 TEST_SLAVE, 
                                 &readConfigReg3.u8, 
                                 CY_S25FS_REG_ADDR_CFR3_V, 
                                 ...
                                 TRX_DATA_WIDTH,
                                 &smifContext);
    CY_ASSERT(writeConfigReg3.field.u1D8h == readConfigReg3.field.u1D8h);
...
void ReadbackXIP(uint16_t rdCmdId, uint32_t size)
{
    ...
    memcpy(readDataBuffer, &pbaseAddrXIP[TEST_PROGRAM_SECTOR], size);
