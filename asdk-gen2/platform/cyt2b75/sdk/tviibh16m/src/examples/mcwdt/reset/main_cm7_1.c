...
int main(void)
{
    ...
    /* If the reset cause is MCWDT2, it goes to infinite loop blinking a LED */
    {
        uint32_t resetReason;
        resetReason = Cy_SysReset_GetResetReason();
        if((resetReason & CY_SYSRESET_MCWDT2) != 0ul)
        {
    ...
    for(;;)
    {
        ...
        uint16_t lowerCounter = Cy_MCWDT_GetLowerLimit(MCWDT2, CY_MCWDT_COUNTER0);
        uint16_t countValue = (uint16_t)Cy_MCWDT_GetCount(MCWDT2, CY_MCWDT_COUNTER0);

        if(countValue > lowerCounter)
        {
            Cy_MCWDT_ClearWatchdog(MCWDT2, CY_MCWDT_COUNTER0);
            Cy_MCWDT_WaitForCounterReset(MCWDT2, CY_MCWDT_COUNTER0);
        }
#endif
    }
}
