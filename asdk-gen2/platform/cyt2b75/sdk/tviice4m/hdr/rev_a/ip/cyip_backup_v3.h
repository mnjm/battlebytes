...
/**
  * \brief Clock Supervision Reference Control (BACKUP_CSV_BAK_CSV_REF_CTL)
  */
typedef struct stc_BACKUP_CSV_BAK_CSV_REF_CTL_field {
  uint32_t                         u9STARTUP:9;
  uint32_t                         :22;
  uint32_t                         u1CSV_EN:1;
} stc_BACKUP_CSV_BAK_CSV_REF_CTL_field_t;

typedef union un_BACKUP_CSV_BAK_CSV_REF_CTL {
  uint32_t                         u32Register;
  stc_BACKUP_CSV_BAK_CSV_REF_CTL_field_t stcField;
  uint8_t                          au8Byte[4];
  uint16_t                         au16Halfword[2];
} un_BACKUP_CSV_BAK_CSV_REF_CTL_t;

/**
  * \brief Clock Supervision Reference Limits (BACKUP_CSV_BAK_CSV_REF_LIMIT)
  */
typedef struct stc_BACKUP_CSV_BAK_CSV_REF_LIMIT_field {
  uint32_t                         u8LOWER:8;
  uint32_t                         :8;
  uint32_t                         u8UPPER:8;
  uint32_t                         :8;
} stc_BACKUP_CSV_BAK_CSV_REF_LIMIT_field_t;

typedef union un_BACKUP_CSV_BAK_CSV_REF_LIMIT {
  uint32_t                         u32Register;
  stc_BACKUP_CSV_BAK_CSV_REF_LIMIT_field_t stcField;
  uint8_t                          au8Byte[4];
  uint16_t                         au16Halfword[2];
} un_BACKUP_CSV_BAK_CSV_REF_LIMIT_t;

/**
  * \brief Clock Supervision Monitor Control (BACKUP_CSV_BAK_CSV_MON_CTL)
  */
typedef struct stc_BACKUP_CSV_BAK_CSV_MON_CTL_field {
  uint32_t                         u8PERIOD:8;
  uint32_t                         :24;
} stc_BACKUP_CSV_BAK_CSV_MON_CTL_field_t;

typedef union un_BACKUP_CSV_BAK_CSV_MON_CTL {
  uint32_t                         u32Register;
  stc_BACKUP_CSV_BAK_CSV_MON_CTL_field_t stcField;
  uint8_t                          au8Byte[4];
  uint16_t                         au16Halfword[2];
} un_BACKUP_CSV_BAK_CSV_MON_CTL_t;
...
/**
  ...
  */
typedef struct stc_BACKUP_CSV_BAK_CSV {
  un_BACKUP_CSV_BAK_CSV_REF_CTL_t  unREF_CTL;          /*!< 0x00000000 Clock Supervision Reference Control */
  un_BACKUP_CSV_BAK_CSV_REF_LIMIT_t unREF_LIMIT;       /*!< 0x00000004 Clock Supervision Reference Limits */
  un_BACKUP_CSV_BAK_CSV_MON_CTL_t  unMON_CTL;          /*!< 0x00000008 Clock Supervision Monitor Control */
  uint32_t                         u32Reserved;
} stc_BACKUP_CSV_BAK_CSV_t;                            /*!< Size = 16 (0x10) */

/**
  * \brief CSV registers for clk_bak clock (BACKUP_CSV_BAK)
  */
typedef struct stc_BACKUP_CSV_BAK {
  stc_BACKUP_CSV_BAK_CSV_t         CSV;                /*!< 0x00000000 clk_bak Backup domain Clock Supervisor registers */
} stc_BACKUP_CSV_BAK_t;                                /*!< Size = 16 (0x10) */

/**
  * \brief SRSS Backup Domain (ver3p3) (BACKUP)
  */
typedef struct stc_BACKUP {
  ...
  uint32_t                         au32Reserved3[29];
  stc_BACKUP_CSV_BAK_t             CSV_BAK;            /*!< 0x00000100 CSV registers for clk_bak clock */
  uint32_t                         au32Reserved4[956];
...
#define BACKUP_CSV_BAK_CSV_REF_CTL_STARTUP_Msk  0x1FFUL
