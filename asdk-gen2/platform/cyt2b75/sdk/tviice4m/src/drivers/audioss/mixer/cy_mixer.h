...
__STATIC_INLINE uint32_t Cy_Mixer_Dst_FIFO_Used(volatile stc_MIXER_MIXER_DST_STRUCT_t * base)
{
    return (base->unDST_FIFO_STATUS.stcField.u7USED);
}
...
__STATIC_INLINE void Cy_Mixer_Tx_Replay(volatile stc_MIXER_MIXER_TX_STRUCT_t * base, bool state)
{
    if(state == true)
    {
        //rmkn: u1REPLAY is not supported base->unTX_FIFO_CTL.stcField.u1REPLAY = true;
    }
    else // (state == false)
    {
        //rmkn: u1REPLAY is not supported base->unTX_FIFO_CTL.stcField.u1REPLAY = false;
    }
}
