...
int main(void)
{
    ...
    { 
        /* Start receiving */
        Cy_SCB_UART_Receive(CY_BSP_USB_SCB_UART_TYPE, &g_uart_in_data[0], UART_RX_UNIT_BYTE, &g_stc_uart_context);

        Cy_SysPm_Sleep(CY_SYSPM_WAIT_FOR_INTERRUPT);

        /* Wait for receive done */
        while((Cy_SCB_UART_GetReceiveStatus(CY_BSP_USB_SCB_UART_TYPE,&g_stc_uart_context) & CY_SCB_UART_RECEIVE_ACTIVE) == CY_SCB_UART_RECEIVE_ACTIVE);
...
void UART_Init_With_Interrupt(uint32_t boadrate, uint32_t sourceFreq)
{
    ...
    Cy_SCB_UART_RegisterCallback(CY_BSP_USB_SCB_UART_TYPE, Scb_UART_Event, &g_stc_uart_context);
    Cy_SCB_UART_Enable(CY_BSP_USB_SCB_UART_TYPE);

    /* Clock Configuration for UART */
    // Assign a programmable divider
    Cy_SysClk_PeriphAssignDivider(CY_BSP_USB_SCB_UART_PCLK, CY_SYSCLK_DIV_24_5_BIT, 0ul);
    // Set divider value
    {
...
void Scb_UART_Event(uint32_t locEvents)
{
    ...
    case CY_SCB_UART_RECEIVE_DONE_EVENT:
        ...
        Cy_SCB_UART_Transmit(CY_BSP_USB_SCB_UART_TYPE, &g_uart_in_data[0], UART_RX_UNIT_BYTE, &g_stc_uart_context);
        break;
