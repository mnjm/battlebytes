...
{
    .sysIntSrc  = CY_BSP_AUDIOSS_PWM0_IRQN,
...
void AudioPwmInterruptHandler(void)
{
    ...
    maskedIntrStatus = Cy_Pwm_Tx_GetInterruptStatusMasked(CY_BSP_AUDIOSS_PWM0_TYPE);
    if((maskedIntrStatus & CY_PWM_INTR_TX_FIFO_TRIGGER) != 0)
    {
        Cy_Pwm_Tx_ClearInterrupt(CY_BSP_AUDIOSS_PWM0_TYPE, maskedIntrStatus);
...
{       
    ...
    {
        Cy_Pwm_Tx_WriteData(CY_BSP_AUDIOSS_PWM0_TYPE, pwmData[data_i]);
    }
    //Cy_Pwm_Tx_SetInterruptMask(CY_BSP_AUDIOSS_PWM0_TYPE, CY_PWM_INTR_TX_FIFO_TRIGGER);
    Cy_Pwm_Tx_SetTriggerInterruptMask(CY_BSP_AUDIOSS_PWM0_TYPE);
}
...
{   
    ...
    io_port_pin_cfg.hsiom = CY_BSP_AUDIOSS_PWM0_L1P0_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_BSP_AUDIOSS_PWM0_L1P0_PORT, CY_BSP_AUDIOSS_PWM0_L1P0_PIN, &io_port_pin_cfg);

    // On PSVP : PWM0_LINE1_N0 -- TP240
    io_port_pin_cfg.hsiom = CY_BSP_AUDIOSS_PWM0_L1N0_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_BSP_AUDIOSS_PWM0_L1N0_PORT, CY_BSP_AUDIOSS_PWM0_L1N0_PIN, &io_port_pin_cfg);

    // On PSVP : PWM0_LINE2_P0 -- TP307
    io_port_pin_cfg.hsiom = CY_BSP_AUDIOSS_PWM0_L2P0_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_BSP_AUDIOSS_PWM0_L2P0_PORT, CY_BSP_AUDIOSS_PWM0_L2P0_PIN, &io_port_pin_cfg);

    // On PSVP : PWM0_LINE2_N0 -- TP340
    io_port_pin_cfg.hsiom = CY_BSP_AUDIOSS_PWM0_L2N0_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_BSP_AUDIOSS_PWM0_L2N0_PORT, CY_BSP_AUDIOSS_PWM0_L2N0_PIN, &io_port_pin_cfg);
}
...
int main(void)
{
    ...
    /* Enable CM4. CY_CORTEX_M7_APPL_ADDR is calculated in linker script, check it in case of problems. */
    ...
    Cy_SysClk_HfClkEnable(CY_SYSCLK_HFCLK_4);
    
    /* Port setting for pwm channels */
    InitPwmLines();
    
    /* Initialization for pwm */
    Cy_Pwm_Tx_DeInit(CY_BSP_AUDIOSS_PWM0_TYPE);

    /* Initialize pwm transmittter interface */
    Cy_Pwm_Tx_Init(CY_BSP_AUDIOSS_PWM0_TYPE, &myPwmConfig);
    ...
    /* Enable pwm */
    Cy_Pwm_Tx_Enable(CY_BSP_AUDIOSS_PWM0_TYPE);
