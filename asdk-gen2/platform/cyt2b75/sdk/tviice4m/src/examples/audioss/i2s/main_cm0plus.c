...
void TransmitI2sData(void)
{
    ...
    for(uint8_t data_i = 0; data_i < TDM_FIFO_THRESHOLD; data_i+=2)
    {
        ...
        Cy_I2S_WriteTxData(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT, txData);
...
void ReceiveI2sData(void)
{
    ...
    for(uint8_t data_i = 0; data_i < TDM_FIFO_THRESHOLD; data_i+=2)
    {
        rxData = Cy_I2S_ReadRxData(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT);
#if CY_I2S_APP_FUNCTION == CY_I2S_RX_TX_LOOP_BACK
        SoundRingBuffer_Push(rxData);
#endif
        Cy_I2S_ReadRxData(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT); // throw garbage
    }
}
...
void AudioSSInterruptHandlerTx(void)
{
    ...
    TX_maskedIntrStatus = Cy_I2S_GetTxInterruptStatusMasked(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT);

    // only FIFO TRIGGER is acceptable
    CY_ASSERT((Cy_I2S_GetTxInterrupt(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT) & ~CY_I2S_INTR_TX_FIFO_TRIGGER) == 0);

    TransmitI2sData();

    Cy_I2S_ClearTxInterrupt(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT, TX_maskedIntrStatus);
}

void AudioSSInterruptHandlerRx(void)
{
    uint32_t RX_maskedIntrStatus;
    RX_maskedIntrStatus = Cy_I2S_GetRxInterruptStatusMasked(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT);

    // only FIFO TRIGGER is acceptable
    CY_ASSERT((Cy_I2S_GetRxInterrupt(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT) & ~CY_I2S_INTR_RX_FIFO_TRIGGER) == 0);

    ReceiveI2sData();

    Cy_I2S_ClearRxInterrupt(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT, RX_maskedIntrStatus);
}
...
int main(void)
{
    ...
    /********************************************/ 
    /* Init TX interface */
    Cy_I2S_Tx_Init(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT, &config_I2S_TX);
    TransmitI2sData(); // Prepare first data into FIFO before enable I2S TX
    Cy_I2S_SetTxTriggerInterruptMask(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT);

    /* Enable TX interface */
    Cy_I2S_EnableTx(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT);

    /* Enable RX interface */
#if CY_I2S_APP_FUNCTION == CY_I2S_RX_TX_LOOP_BACK
    /* Init RX interface */
    Cy_I2S_Rx_Init(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT, &config_I2S_RX);
    Cy_I2S_SetRxTriggerInterruptMask(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT);

    Cy_I2S_EnableRx(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT);
#endif
    for(;;);
}
/*******************************************************************************
