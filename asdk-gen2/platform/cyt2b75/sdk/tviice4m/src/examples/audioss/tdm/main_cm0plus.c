...
//  {                       portReg,                       pinNum, outVal,                driveMode,                            hsiom, intEdge, intMask, vtrip, slewRate, driveSel },
    {   CY_BSP_AUDIOSS_TDM0_TX_SCK_PORT,   CY_BSP_AUDIOSS_TDM0_TX_SCK_PIN,    0ul, CY_GPIO_DM_STRONG_IN_OFF,   CY_BSP_AUDIOSS_TDM0_TX_SCK_PIN_MUX,     0ul,     0ul,   0ul,      0ul,      0ul },
    { CY_BSP_AUDIOSS_TDM0_TX_FSYNC_PORT, CY_BSP_AUDIOSS_TDM0_TX_FSYNC_PIN,    0ul, CY_GPIO_DM_STRONG_IN_OFF, CY_BSP_AUDIOSS_TDM0_TX_FSYNC_PIN_MUX,     0ul,     0ul,   0ul,      0ul,      0ul },
    {    CY_BSP_AUDIOSS_TDM0_TX_SD_PORT,    CY_BSP_AUDIOSS_TDM0_TX_SD_PIN,    0ul, CY_GPIO_DM_STRONG_IN_OFF,    CY_BSP_AUDIOSS_TDM0_TX_SD_PIN_MUX,     0ul,     0ul,   0ul,      0ul,      0ul },
    {   CY_BSP_AUDIOSS_TDM0_RX_SCK_PORT,   CY_BSP_AUDIOSS_TDM0_RX_SCK_PIN,    0ul, CY_GPIO_DM_STRONG_IN_OFF,   CY_BSP_AUDIOSS_TDM0_RX_SCK_PIN_MUX,     0ul,     0ul,   0ul,      0ul,      0ul },
    { CY_BSP_AUDIOSS_TDM0_RX_FSYNC_PORT, CY_BSP_AUDIOSS_TDM0_RX_FSYNC_PIN,    0ul, CY_GPIO_DM_STRONG_IN_OFF, CY_BSP_AUDIOSS_TDM0_RX_FSYNC_PIN_MUX,     0ul,     0ul,   0ul,      0ul,      0ul },
    {    CY_BSP_AUDIOSS_TDM0_RX_SD_PORT,    CY_BSP_AUDIOSS_TDM0_RX_SD_PIN,    0ul,         CY_GPIO_DM_HIGHZ,    CY_BSP_AUDIOSS_TDM0_RX_SD_PIN_MUX,     0ul,     0ul,   0ul,      0ul,      0ul },
    {   CY_BSP_AUDIOSS_TDM0_RX_MCK_PORT,   CY_BSP_AUDIOSS_TDM0_RX_MCK_PIN,    0ul,         CY_GPIO_DM_HIGHZ,   CY_BSP_AUDIOSS_TDM0_RX_MCK_PIN_MUX,     0ul,     0ul,   0ul,      0ul,      0ul },
#if (CY_TDM_USE_CLK == CY_TDM_INTERNAL_CLOCK)
    {   CY_BSP_AUDIOSS_TDM0_TX_MCK_PORT,   CY_BSP_AUDIOSS_TDM0_TX_MCK_PIN,    0ul, CY_GPIO_DM_STRONG_IN_OFF,   CY_BSP_AUDIOSS_TDM0_TX_MCK_PIN_MUX,     0ul,     0ul,   0ul,      0ul,      0ul },
#else
    {   CY_BSP_AUDIOSS_TDM0_TX_MCK_PORT,   CY_BSP_AUDIOSS_TDM0_TX_MCK_PIN,    0ul,         CY_GPIO_DM_HIGHZ,   CY_BSP_AUDIOSS_TDM0_TX_MCK_PIN_MUX,     0ul,     0ul,   0ul,      0ul,      0ul },
#endif
};
...
{
    .sysIntSrc  = CY_BSP_AUDIOSS_TDM0_TX_IRQN,
...
{
    .sysIntSrc  = CY_BSP_AUDIOSS_TDM0_RX_IRQN,
    .intIdx     = CPUIntIdx4_IRQn,
    .isEnabled  = true,
};
...
void TransmitTdmData(void)
{
    ...
    for(uint8_t data_i = 0; data_i < TDM_FIFO_THRESHOLD; data_i+=TDM_CNANNEL_COUNT)
    {
        ...
        Cy_AudioTDM_WriteTxData(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT, txData);
...
void ReceiveTdmData(void)
{
    ...
    for(uint8_t data_i = 0; data_i < TDM_FIFO_THRESHOLD; data_i+=TDM_CNANNEL_COUNT)
    {
        rxData = Cy_AudioTDM_ReadRxData(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT);
#if CY_TDM_APP_FUNCTION == CY_TDM_RX_TX_LOOP_BACK
        ...
        {
            Cy_AudioTDM_ReadRxData(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT); // throw garbage
        }
    }
}
...
void AudioSSInterruptHandlerTx(void)
{
    ...
    TX_maskedIntrStatus = Cy_AudioTDM_GetTxInterruptStatusMasked(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT);

    // only FIFO TRIGGER is acceptable
    CY_ASSERT((Cy_AudioTDM_GetTxInterrupt(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT) & ~CY_TDM_INTR_TX_FIFO_TRIGGER) == 0);

    TransmitTdmData();

    Cy_AudioTDM_ClearTxInterrupt(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT, TX_maskedIntrStatus);
}

void AudioSSInterruptHandlerRx(void)
{
    uint32_t RX_maskedIntrStatus;
    RX_maskedIntrStatus = Cy_AudioTDM_GetRxInterruptStatusMasked(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT);

    // only FIFO TRIGGER is acceptable
    CY_ASSERT((Cy_AudioTDM_GetRxInterrupt(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT) & ~CY_TDM_INTR_RX_FIFO_TRIGGER) == 0);

    ReceiveTdmData();

    Cy_AudioTDM_ClearRxInterrupt(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT, RX_maskedIntrStatus);
}
...
int main(void)
{
    ...
    /* Init TX interface */
    Cy_AudioTDM_DeInit(CY_BSP_AUDIOSS_TDM0_TYPE);
    Cy_AudioTDM_Init(CY_BSP_AUDIOSS_TDM0_TYPE, &config_tdm);
    TransmitTdmData(); // Prepare first data into FIFO before enable TDM TX
    Cy_AudioTDM_SetTxTriggerInterruptMask(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT);

    /* Enable TX interface */
    Cy_AudioTDM_StartTx(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_TX_STRUCT);

    /* Enable RX interface */
#if CY_TDM_APP_FUNCTION == CY_TDM_RX_TX_LOOP_BACK
    /* Init RX interface */
    Cy_AudioTDM_SetRxTriggerInterruptMask(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT);

    /* Enable RX interface */
    Cy_AudioTDM_StartRx(&CY_BSP_AUDIOSS_TDM0_TYPE->TDM_RX_STRUCT);
#endif
    for(;;);
}
