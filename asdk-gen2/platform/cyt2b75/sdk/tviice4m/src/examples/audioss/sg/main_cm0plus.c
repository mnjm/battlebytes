...
{
    .sysIntSrc  = CY_BSP_AUDIOSS_SG0_IRQN,
...
void SG_UserInterruptHandler(void)
{
    ...
    maskedIntrStatus = Cy_SG_GetInterruptStatusMasked(CY_BSP_AUDIOSS_SG0_TYPE);
    
    if((maskedIntrStatus & CY_SG_INTR_TX_COMPLETE) != 0)
    {
        {  
            Cy_SG_PushSegment(CY_BSP_AUDIOSS_SG0_TYPE, userData, &sgContext);
        }
    }
     Cy_SG_ClearInterrupt(CY_BSP_AUDIOSS_SG0_TYPE, maskedIntrStatus);
}
...
int main(void)
{
    ...
    // CY_BSP_AUDIOSS_SG0_TYPE; /* Uncomment this to check registers when debug */
    
    /**************************************************/
    /*******    Clock divider setting       ***********/
    /**************************************************/
    Cy_SysClk_HfClkEnable(CY_SYSCLK_HFCLK_2); /* Enable hfclk2 for Audio */
    Cy_SysClk_HfClockSetSource(CY_SYSCLK_HFCLK_2, CY_SYSCLK_HFCLK_IN_CLKPATH1);
    Cy_SysClk_HfClockSetDivider(CY_SYSCLK_HFCLK_2, CY_SYSCLK_HFCLK_NO_DIVIDE);
    
    /**************************************/
    /* Port Setting for SG transmission   */
    /**************************************/
    SG_port_pin_cfg.driveMode = CY_GPIO_DM_STRONG_IN_OFF; 
    SG_port_pin_cfg.hsiom = CY_BSP_AUDIOSS_SG0_AMPL_PIN_MUX; 
    Cy_GPIO_Pin_Init(CY_BSP_AUDIOSS_SG0_AMPL_PORT, CY_BSP_AUDIOSS_SG0_AMPL_PIN, &SG_port_pin_cfg); 
    
    SG_port_pin_cfg.driveMode = CY_GPIO_DM_STRONG_IN_OFF;
    SG_port_pin_cfg.hsiom = CY_BSP_AUDIOSS_SG0_TONE_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_BSP_AUDIOSS_SG0_TONE_PORT, CY_BSP_AUDIOSS_SG0_TONE_PIN, &SG_port_pin_cfg);
    ...
    Cy_SG_DeInit(CY_BSP_AUDIOSS_SG0_TYPE);
    Cy_SG_Init(CY_BSP_AUDIOSS_SG0_TYPE, &sgInitConfig);
    Cy_SG_EnableSource(CY_BSP_AUDIOSS_SG0_TYPE);
    Cy_SG_SetInterruptMask(CY_BSP_AUDIOSS_SG0_TYPE, CY_SG_INTR_TX_COMPLETE);
    Cy_SG_StartSegment(CY_BSP_AUDIOSS_SG0_TYPE, userData, SIZE_OF_SEGMENT, &sgContext);
