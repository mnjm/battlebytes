...
int main(void)
{
    ...
    Cy_SCB_UART_DeInit(CY_BSP_USB_SCB_UART_TYPE);
    Cy_SCB_UART_Init(CY_BSP_USB_SCB_UART_TYPE, &g_stc_uart_config, &g_stc_uart_context);
    Cy_SCB_UART_RegisterCallback(CY_BSP_USB_SCB_UART_TYPE, (scb_uart_handle_events_t)Scb_UART_Event, &g_stc_uart_context);
#if defined(E_UART_ECHO_INTR_RINGBUF)
    Cy_SCB_UART_StartRingBuffer(CY_BSP_USB_SCB_UART_TYPE, (void *)g_uart_rx_ring, E_UART_RING_BUF_SIZE, &g_stc_uart_context);
#endif
    Cy_SCB_UART_Enable(CY_BSP_USB_SCB_UART_TYPE);
    ...
    /* (3) Interrupt & Receive by threshold byte unit (E_UART_RECV_THRESHOLD) */
    Cy_SCB_UART_Receive(CY_BSP_USB_SCB_UART_TYPE, g_uart_in_data, E_UART_RECV_THRESHOLD, &g_stc_uart_context);
#endif
    ...
    for(;;)
    {
        ...
        uint32_t num = Cy_SCB_UART_GetNumInRingBuffer(CY_BSP_USB_SCB_UART_TYPE, &g_stc_uart_context);
        if (num != 0) {
            Cy_SCB_UART_Receive(CY_BSP_USB_SCB_UART_TYPE, &g_uart_user_buf[0], num, &g_stc_uart_context);
            Cy_SCB_UART_Transmit(CY_BSP_USB_SCB_UART_TYPE, &g_uart_user_buf[0], num, &g_stc_uart_context);
        }
#endif
    }
}
...
void Peripheral_Initialization(void)
{
    ...
    stc_port_pin_cfg_uart.hsiom     = CY_BSP_USB_SCB_UART_RX_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_BSP_USB_SCB_UART_RX_PORT, CY_BSP_USB_SCB_UART_RX_PIN, &stc_port_pin_cfg_uart);

    /* P13.1 -> scb[3].uart_tx */
    stc_port_pin_cfg_uart.driveMode = CY_GPIO_DM_STRONG_IN_OFF;
    stc_port_pin_cfg_uart.hsiom     = CY_BSP_USB_SCB_UART_TX_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_BSP_USB_SCB_UART_TX_PORT, CY_BSP_USB_SCB_UART_TX_PIN, &stc_port_pin_cfg_uart);
    ...
    Cy_SysClk_PeriphAssignDivider(CY_BSP_USB_SCB_UART_PCLK, CY_SYSCLK_DIV_24_5_BIT, 0u);
    ...
    /* Int1 -> CY_BSP_USB_SCB_UART_TYPE */
    stc_sysint_irq_cfg_uart.sysIntSrc = CY_BSP_USB_SCB_UART_IRQN;
...
void Scb_UART_IntrISR(void)
{
    ...
    /* UART Echo Test (High-Level)            */
    /* (2) Interrupt & Receive by 1 byte unit */
    uint32_t num = Cy_SCB_UART_GetNumInRxFifo(CY_BSP_USB_SCB_UART_TYPE);
    if (num != 0) {
        Cy_SCB_UART_Receive(CY_BSP_USB_SCB_UART_TYPE, &g_uart_in_data[0], num, &g_stc_uart_context);
        Cy_SCB_UART_Transmit(CY_BSP_USB_SCB_UART_TYPE, &g_uart_in_data[0], num, &g_stc_uart_context);
        Cy_SCB_SetRxFifoLevel(CY_BSP_USB_SCB_UART_TYPE, 0);
    }
#endif

    /* UART interrupt handler */
    Cy_SCB_UART_Interrupt(CY_BSP_USB_SCB_UART_TYPE, &g_stc_uart_context);
...
void Scb_UART_Event(uint32_t locEvents)
{
    switch (locEvents) {
        ...
        case CY_SCB_UART_RECEIVE_DONE_EVENT:
            ...
            /* (3) Interrupt & Receive by threshold byte unit (E_UART_RECV_THRESHOLD) */
            Cy_SCB_UART_Transmit(CY_BSP_USB_SCB_UART_TYPE, &g_uart_in_data[0], E_UART_RECV_THRESHOLD, &g_stc_uart_context);
            Cy_SCB_UART_Receive(CY_BSP_USB_SCB_UART_TYPE, &g_uart_in_data[0], E_UART_RECV_THRESHOLD, &g_stc_uart_context);
#endif
            /* Re-Enable Interrupt */
            Cy_SCB_SetRxInterruptMask(CY_BSP_USB_SCB_UART_TYPE, g_stc_uart_config.rxFifoIntEnableMask);
            break;
...
void Term_Printf(void *fmt, ...)
{
    ...
    while (Cy_SCB_UART_IsTxComplete(CY_BSP_USB_SCB_UART_TYPE) != true) {};
    Cy_SCB_UART_PutArray(CY_BSP_USB_SCB_UART_TYPE, g_uart_out_data, strlen((char *)g_uart_out_data));
    va_end(arg);
}
...
void Term_Input(void)
{
    ...
    /* Check Receive Data */
    rx_num = Cy_SCB_UART_GetNumInRxFifo(CY_BSP_USB_SCB_UART_TYPE);
    if (0u == rx_num) return;

    /* UART In/Out */
#if 0
    uint32_t    cmd;
    cmd = Cy_SCB_UART_Get(CY_BSP_USB_SCB_UART_TYPE);
    while (0UL == Cy_SCB_UART_Put(CY_BSP_USB_SCB_UART_TYPE, cmd)) {}
#endif
#if 1
    Cy_SCB_UART_GetArray(CY_BSP_USB_SCB_UART_TYPE, g_uart_in_data, rx_num);
    Cy_SCB_UART_PutArray(CY_BSP_USB_SCB_UART_TYPE, g_uart_in_data, rx_num);
#endif
#if 0
    Cy_SCB_UART_GetArrayBlocking(CY_BSP_USB_SCB_UART_TYPE, g_uart_in_data, rx_num);
    Cy_SCB_UART_PutArrayBlocking(CY_BSP_USB_SCB_UART_TYPE, g_uart_in_data, rx_num);
    Cy_SCB_UART_PutString(CY_BSP_USB_SCB_UART_TYPE, (char_t *)g_uart_in_data);
#endif
}
