...
#define COUNTERPART_PPU_FAULT CY_SYSFLT_PERI_MS_VIO_1 // Counterpart means CM0+
#define COUNTERPART_CPU_ID    CPUSS_MS_ID_CM4

#define INDICATOR_LED_PORT    CY_BSP_USER_LED1_PORT
#define INDICATOR_LED_PIN     CY_BSP_USER_LED1_PIN
#define INDICATOR_LED_MUX     CY_BSP_USER_LED1_PIN_MUX
...
void irqFaultReport0Handler(void)
{
    ...
    if(TP_PRIVIREGED == 0)
    {
        CY_ASSERT(violatingInfo.bitfld.user_read    == 1); // the fault should occur in read process
        CY_ASSERT(violatingInfo.bitfld.user_write   == 0);
        CY_ASSERT(violatingInfo.bitfld.user_execute == 0);
    }
    else
    {
        CY_ASSERT(violatingInfo.bitfld.privileged_read    == 1);  // the fault should occur in read process
