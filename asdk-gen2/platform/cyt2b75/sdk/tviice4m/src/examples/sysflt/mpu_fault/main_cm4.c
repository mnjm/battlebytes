...
void irqFaultReport0Handler(void)
{
    ...
    {
        if(status == CY_SYSFLT_CPUSS_MPU_VIO_0) // Master 0 (CM0+) PPU fault
        {
            ...
            for(;;)
            {
                ...
                Cy_GPIO_Inv(CY_BSP_USER_LED2_PORT, CY_BSP_USER_LED2_PIN);
            }
        }
    }
...
int main(void)
{
    ...
    user_led_port_pin_cfg.hsiom  = CY_BSP_USER_LED2_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_BSP_USER_LED2_PORT, CY_BSP_USER_LED2_PIN, &user_led_port_pin_cfg);
