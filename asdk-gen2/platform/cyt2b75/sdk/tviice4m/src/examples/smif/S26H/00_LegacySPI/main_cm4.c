...
int main(void)
{
...
#if 1 // If your device is hyper bus mode by default, please enable this code.
    // Assumed the device is in Hyper bus mode by default
    EnterSPIMode(SMIF_DEVICE_USED);
#endif
    ...
    /************************************/
    Cy_SMIF_DeviceSetDataSelect(SMIF_DEVICE_USED, CY_SMIF_DATA_SEL0);
    Cy_SMIF_DeviceWriteEnable(SMIF_DEVICE_USED);
    ...
    CY_ASSERT(Cy_SMIF_InitXIPModeForExMem(SMIF_DEVICE_USED, CY_SMIF_DEV_SINGLE_BIT_0To1, &smifDev0Cfg, &smifDevice0Context) == CY_SMIF_SUCCESS);
