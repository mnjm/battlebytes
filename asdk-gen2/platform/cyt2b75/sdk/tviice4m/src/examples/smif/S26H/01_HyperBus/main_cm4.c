...
#define TARGET_CLK_FREQ_IN_MHZ (100)
/**********************************************/
...
int main(void)
{
    ...
    Cy_SysClk_HfClkEnable(SMIF_HF_CLOCK);   // 200MHz
    ...
    cy_en_smif_slave_select_t slaveSelect = (SMIF_DEVICE_USED == SMIF0_DEVICE0)
    ...
    flashVCR1.fld.driveStrength         = CY_SMIF_HB_S26H_DRV_60_OHMS;
    ...
    /*********  Write Data to the Top Sector   *******/
    for(uint32_t i_WriteAddr = 0; i_WriteAddr < (TEST_PROGRAM_SIZE / 2);i_WriteAddr += HB_WRITE_BUF_SIZE_WORD)
    {
        // write buffer size = 512 byte = 256 word
    ...
    for(uint32_t i_WriteAddr = 0; i_WriteAddr < (TEST_PROGRAM_SIZE / 2);i_WriteAddr += READ_BUF_SIZE_IN_HWORD)
    {
...
static void EnterHyperBusMode(volatile cy_stc_smif_reg_device_t *device)
{
    cy_en_smif_slave_select_t slaveSelect = (device == SMIF0_DEVICE0)
                                          ? CY_SMIF_SLAVE_SELECT_0
                                          : CY_SMIF_SLAVE_SELECT_1;
