...
uint8_t SMIF_Crypto_Input[] =
{
    ...
    0x11u,0x00u,0x00u,0x00u,0x6bu,0xc1u,0xbeu,0xe2u,0x2eu,0x40u,0x9fu,0x96u,0xe9u,0x3du,0x7eu,0x11u,
};
...
static void TestFor_DQUAD_CryptoMMIOWrite_Then_RawAndCryptoXIPRead(uint16_t rd_cmdId);
static void TestFor_DQUAD_CryptoXIPWrite_Then_RawAndCryptoXIPRead(uint16_t rd_cmdId);
...
int main(void)
{
    ...
    /*********************************************/
    Cy_SMIF_SetCryptoInput128(CY_SMIF_USED, (uint8_t*)SMIF_Crypto_Input);
    Cy_SMIF_SetCryptoKey128(CY_SMIF_USED, AesEcbKey128);
    memcpy(cryptedProgramData, programData, TEST_PROGRAM_SIZE);
    Cy_SMIF_Encrypt(CY_SMIF_USED,(CY_SMIF_GetXIP_Address(CY_SMIF_USED) + TEST_DQUAD_XIP_PROGRAM_ADDR), cryptedProgramData, TEST_PROGRAM_SIZE,&smifContext);
    ...
    TestFor_DQUAD_CryptoMMIOWrite_Then_RawAndCryptoXIPRead(CY_SMIF_S25FL_ID_4QIOR);

    smifDevDualQuadCfg.xipReadCmdId = CY_SMIF_S25FL_ID_4QIOR;
    smifDevDualQuadCfg.xipReadMode  = CY_SMIF_S25FL_RW_NON_CONTINUOUS;
    Cy_SMIF_InitXIPModeForExMem(SMIF0_DEVICE0, CY_SMIF_DEV_D_QUAD_BIT_0To3, &smifDevDualQuadCfg, &smifDeviceContext);
    Cy_SMIF_InitXIPModeForExMem(SMIF0_DEVICE1, CY_SMIF_DEV_D_QUAD_BIT_4To7, &smifDevDualQuadCfg, &smifDeviceContext);
    TestFor_DQUAD_CryptoXIPWrite_Then_RawAndCryptoXIPRead(CY_SMIF_S25FL_ID_4QIOR);

    // Reaching here means this example worked properly.
    for(;;);
}

// In this function, at first, write the data which is crypted by Cy_SMIF_Encrypt function in advance in MMIO mode.
// Then read the Data with decription on the fly in XIP mode.
static void TestFor_DQUAD_CryptoMMIOWrite_Then_RawAndCryptoXIPRead(uint16_t rd_cmdId)
{
    ...
    for(uint32_t i_WriteOffsetAddrOfQuadDevice = 0; i_WriteOffsetAddrOfQuadDevice < (programSizeInByte / 2); i_WriteOffsetAddrOfQuadDevice+=DEVICE_PROGRAM_BUFFER_SIZE)
    {
        ...
        // Issue program
        Cy_SMIF_MMIO_Program_ExMem(CY_SMIF_USED, D_QUAD_SLAVES, CY_SMIF_S25FL_ID_4QPP, writeStartAddress, (DEVICE_PROGRAM_BUFFER_SIZE*2)/* 512B * 2 */, (uint8_t*)&cryptedProgramData[i_WriteOffsetAddrOfQuadDevice * 2], CY_SMIF_BLOCKING, TEST_SPI_WRITE_LC, 0, &smifDeviceContext, &smifContext);
    ...
    /***************************/
    Cy_SMIF_DeviceCryptoDisable(SMIF0_DEVICE0);
    Cy_SMIF_DeviceCryptoDisable(SMIF0_DEVICE1);
    ...
    // Check read data
    CY_ASSERT_L1((memcmp(readDataBuffer, cryptedProgramData, TEST_PROGRAM_SIZE) == 0));

    /**************************/
    /** Read with decryption **/
    /**************************/
    Cy_SMIF_DeviceCryptoEnable(SMIF0_DEVICE0);
    Cy_SMIF_DeviceCryptoEnable(SMIF0_DEVICE1);
