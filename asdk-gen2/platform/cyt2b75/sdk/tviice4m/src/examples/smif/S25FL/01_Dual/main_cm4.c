...
int main(void)
{
    ...
    smifDevDual0Cfg.addr = CY_SMIF_GetXIP_Address(CY_SMIF_USED);
    smifDevDual1Cfg.addr = CY_SMIF_GetXIP_Address(CY_SMIF_USED) + 0x02000000;
    /*********************************************************/
    ...
    Cy_SMIF_InitXIPModeForExMem(SMIF0_DEVICE0, CY_SMIF_DEV_DUAL_BIT_0To1, &smifDevDual0Cfg, &smifDevice0Context);
    Cy_SMIF_InitXIPModeForExMem(SMIF0_DEVICE1, CY_SMIF_DEV_DUAL_BIT_4To5, &smifDevDual1Cfg, &smifDevice1Context);
...
static void TestFor_MMIO_WRITE_Then_XIP_CONTINUOUS_READ(cy_en_smif_slave_select_t slave, uint16_t wt_cmdId, uint16_t rd_cmdId)
{
    ...
    Cy_SMIF_MMIO_Read_ExMem(CY_SMIF_USED, slave, rd_cmdId, TEST_PROGRAM_SECTOR, 4u, (uint8_t*)&dummyRead, TEST_SPI_READ_LC, CY_SMIF_S25FL_RW_CONTINUOUS, CY_SMIF_BLOCKING, smifDeviceContext, &smifContext);
    ...
    Cy_SMIF_MMIO_Read_ExMem(CY_SMIF_USED, slave, rd_cmdId, TEST_PROGRAM_SECTOR, 4u, (uint8_t*)&dummyRead, TEST_SPI_READ_LC, CY_SMIF_S25FL_RW_NON_CONTINUOUS, CY_SMIF_BLOCKING, smifDeviceContext, &smifContext);
